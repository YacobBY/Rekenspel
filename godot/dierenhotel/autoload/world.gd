...
func stappen(id: String, punten: Array, o: Dictionary = {}) -> bool:
	...
	if str(o.get("pose", "")) in ["", "sjok"] \
			...
			and not rust():
		# a walk (also the slow sulk out of a shop, `sjok`): every leg goes
		# round what stands in the room and round the water (`looppad`).  A
		# walk that counts its own points keeps them, or the count would be
		# off; swimming and jumping keep their exact line; reduced motion puts
		# him on the last point anyway.
		var weg: Array = []
