...
func test_gekocht_bed_en_de_kar_staan_in_de_weg() -> void:
	...
	# across the bed, wherever the furniture rules put it
	var mid := Vector2(float(bed.get("x", 60.0)), float(bed.get("z", 72.0)))
	var a := mid + Vector2(0.0, -26.0)
	var b := mid + Vector2(0.0, 26.0)
...
func test_dwalen_kiest_alleen_haalbare_plekken() -> void:
	...
	for p in r.plekken:
		if _in(afdrukken, Vector2(p[0], p[1])) != "" \
			...
			in_iets += 1
	waar(in_iets > 0, "er liggen dwaalplekken tegen of in een bed (%d)" % in_iets)
