...
sorteerronde vóór de eerste som is een overtreding. *(Eigenaarswens 2026-09-23: moet
het dier van de beurt de kamer van het spel nog in lopen, dan telt die seconde vanaf
zijn aankomst — "Zorg dat de minigame pas begint wanneer het dier er is"; tot dan het
hotelwolkje `komt eraan` met `👀 Volg`, world.md §5.8.)*
...
- [x] omvang **M** · model **opus** · hangt af van: — · V1 beslist tijdens de bouw (§6)
...
- [x] omvang **M** · model **opus** · hangt af van: — · V4 beslist tijdens de bouw (§6)
...
- [x] omvang **M** · model **opus** · hangt af van: `B2`
...
- [x] omvang **M** · model **opus** · hangt af van: `V1` · **klaar 2026-09-20, lokaal model (Qwen3.8-Flash-Next)**
...
- [x] omvang **S** · model **opus** · hangt af van: — · **klaar 2026-09-20, lokaal model (Qwen3.8-Flash-Next)**
...
- [x] omvang **M** · model **opus** · hangt af van: — · **klaar 2026-09-20, lokaal model (Qwen3.8-Flash-Next)**
...
- [x] omvang **M** · model **opus** · hangt af van: — · **klaar 2026-09-21, lokaal model (Qwen3.8-Flash-Next)**
...
- [x] omvang **S** · model **opus** · hangt af van: `N2` · **klaar 2026-09-21, lokaal model (Qwen3.8-Flash-Next)**
...
#### `R2` — De Speelzaal 🧸 🔎
- [x] omvang **M/L** · model **opus** · hangt af van: `R1` · **klaar 2026-09-21, lokaal model (Qwen3.8-Flash-Next)**
...
#### `S5` — Een misser: het dier is even sip en de vraag komt passend terug ✅
- [x] omvang **M** · model **opus** · hangt af van: `B2` · **eigenaarswens 2026-09-20, doen vóór `B3`** · begin met `git diff tests/test_ui.gd`
...
- [x] omvang **M** · model **opus** · hangt af van: `B3`
...
- **vervallen — de eigenaar overrulet deze taak (2026-09-23):** "Zorg dat de minigame pas begint wanneer het dier er is." De som staat er dus juist níét meteen: zolang de hinkelaar naar zijn steen loopt tekent het spel niets en hangt het hotelwolkje `komt eraan` met `👀 Volg` aan de tuindeur; de kaart komt als hij op zijn steen staat (games-b.md §3.6, world.md §5.8, §7 regel "wacht-op-dier"). Niet uitvoeren.
...
**Nog echt open: `V6`, `V7`, `V10`.** De rest is beslist doordat de bijbehorende
taak gebouwd is; die vragen blijven hieronder staan als verslag van de redenering,
met de uitkomst erboven. Ga er niet opnieuw over in beraad (nagekeken 2026-09-22).

| vraag | stand |
|---|---|
| `V1` zwembad, plons in plaats van eindstreep | **beslist**, gebouwd in `N3` volgens de aanname |
| `V2` balk kost één voxelstap kamer | **beslist**, gebouwd in `B2` volgens de aanname |
| `V3` bedden, de deur is geen controleknop meer | **beslist**, gebouwd in `N9` volgens de aanname |
| `V4` meubels, eerst de buidel tellen | **beslist**, gebouwd in `N6` volgens de aanname |
| `V5` twee of drie nieuwe kamers | **half beslist**: de Speelzaal staat (`R2`); de Kas volgt in `R3`, de Feestzaal komt er niet |
| `V6` lengte van een seizoen | open, raakt `R7` |
| `V7` Bellensprint als apart spel | open, pas te beantwoorden ná `N14` |
| `V8` aaien: drie is het aantal | **beslist**, gebouwd in `D1` volgens de aanname |
| `V9` was, één stuk tegelijk van de berg | **beslist**, gebouwd in `N8` volgens de aanname |
| `V10` pictogram boven de antwoordstrook | open, raakt `B4` — nu aan de beurt |
...
- 2026-09-20 01:40 (nacht, lokaal model) — **N8 — de was begint met een som en heeft halverwege een tweede.** Bestanden: `games/was/spel.gd`, `games/was/test_was.gd`. Nieuwe stap `"vraag0"` vóór `"sorteren"`: band 3/4 vraagt `Hoeveel stuks liggen er?` met `goed = T`, band 5 `Hoeveel blokjes worden dat?` met `regel2` de bestaande 📦-legenda en `goed = T / 2`; knoppen `📊 <n>` via `o["icoon"]`. Nieuwe stap `"vraagT"` zodra `_stuks_over() <= T/2` én `T >= 8` (`TUSSEN_MIN`): `Hoeveel liggen er nog?` met `goed = _stuks_over()`, daarna terug naar `"sorteren"`; het veld `tussen` in de stand zorgt dat hij maar één keer per beurt komt. **De berg is nu 🧹** (`BERG_ICOON`) in plaats van 🧺, want 🧺 draagt al de soort "doeken" (`soorten.gd:21`); de soort zelf hoefde daarvoor niet te veranderen, dus `soorten.gd` staat niet in deze commit terwijl §4 hem wel als N8-bestand noemt. Tijdens de twee bergvragen is de bron `aantal: 0` — `UiBron._get_drag_data` geeft dan `null`, dus de berg is zichtbaar en leesbaar maar nog niet leeg te slepen, en `_pak()` balt zelf af omdat de stap niet `"sorteren"` is; de "pak N"-greep is weg en de titel zegt "kijk er goed naar", want een knop die liegt is erger dan een knop die er niet is. Hulpladder: `Econ.tel_mee(1, …)` over de stuks, `tel_mee(2, …)` over de blokjes, en bij drie missers een spookcijfer óp de berg (`_spook_berg`, prio 3). `_volgende()` vangt de bestemming af vóór zijn `await` en stapt niet meer na als de vraag intussen veranderd is — een oude navolger mocht geen kaart terugbouwen voor een vraag die al weg was. **Drie dingen buiten de letter van §4, hier genoemd:** (1) `_normaliseer` bestaat in dit spel niet, `_herstel()` ís zijn normalisatie, en beide nieuwe stappen en `tussen` zitten daarin; (2) `_pas_in_het_kader()` kreeg een laatste-redmiddel na zijn vier rondes — regel 1 (wolkje weg) en regel 3 (tweede regel van de kaart af) konden samen de legenda volledig laten verdwijnen, en in groep 5 is de vraag zonder legenda niet te beantwoorden, dus hij gaat dan terug op de kaart, dezelfde uitweg die `_weeg_legenda` al had; (3) tijdens vraag0/vraagT staat `_berg_kort` af — de berg met zijn lange zin "🧹 20 nog te sorteren" bezette bandcellen die de kaart met tweede regel nodig had, en de kaart vraagt, dus de berg hoeft niet na te praten; het cijfer blijft, de woorden gaan. **Suite 473 goed, 0 fout** (was 471, +2 nieuwe tests), `DH_TEST_FILTER=was` 21/0, `core/sommen.gd` byte-gelijk, export zonder fouten, beeld `/tmp/n8` (tablet) en `/tmp/n8tel` (telefoon). **Niet gepusht** — de lead controleert.
- 2026-09-20 03:10 (nacht, lokaal model) — **B2-nazorg: de balk meet zichzelf niet meer per frame.** Bestanden: `autoload/ui.gd`, `tests/test_balk.gd`. `Ui._process` riep `_balk_herstel()` elke frame aan; die vraagt `kaart_mat()` aan de gedokte kaart, en die meting wist `custom_minimum_size` en zet de hulpregel aan en uit — dus zolang er een som open stond werd die kaart élk frame opnieuw ingedeeld. Nu gebeurtenisgestuurd: `_balk_herstel()` komt van `somkaart` (was al), van de `on_weg` van de kaart (was al) en van `Kaart.hulp()` (was al), en nieuw van `Kaart.regel()`, `Kaart.regel2()`, `Kaart.som()`, `Kaart.zet()` en `Kaart.klaar()`. Elke gebeurtenis zet één uitgestelde tweede afrekening in aan het eind van zijn eigen frame (`_balk_na_meeting`), want een spel mag de kaart die het net kreeg nog opkleden zonder de balk daarover te spreken — `games/voerkar/spel.gd` verbergt de somregel en het antwoordvakje direct ná `somkaart()`, en zonder die tweede pas zou die kaart te hoog voor het papier zijn blijven staan. Een frame zonder gebeurtenis meet niets. **De guard die deze taak echt nodig had is `_balk_bezig`:** de was beantwoordt `kader_veranderd` door zijn legenda opnieuw te wegen, en die weging roept `Kaart.regel2()`, die de balk op de hoogte stelt — met de nieuwe melding onder `regel2` werd dat `regel2 → _balk_herstel → zet_balk → kader_veranderd → regel2 → …` tot de stack overstroomde (8983 motorfouten in één run, zes was-tests rood). Een melding die binnen de emit-keten van een lopende afrekening binnenkomt wordt nu onthouden in plaats van gehoorzaamd, en `_balk_pas()` rekent maximaal vier keer door tot de kost niet meer verandert. **Waarom `kader_veranderd` níét in de lijst staat**, terwijl §7 hem noemde: `World.meet()` vraagt `Ui.balk_bepaal()` zélf, vóór het de nieuwe kaderhoogte berekent; nog een keer vragen vanuit `kader_veranderd` zou de kaderhoogte die de balk zojuist zelf kleiner maakte terug in het dak van de balk voeden (0.34 van dat kader), en dan gaan de twee elkaar achterna — precies de oscillatie die de guard hierboven afkapt. **Wat de test níét claimt:** `Hits._maat_van` raakt élke kaart met een zichtbare hulplijn, niet alleen de gedokte, en doet dat élke frame dat `Hits.plaats()` draait; dat is de plaatsingslaag eigen behoefte (een kaart die op de verkeerde breedte gemeten wordt groeit uit tot een strook van kaderhoogte) en blijft precies zo — daarom telt de test `Ui._balk_tellingen`, de eigen herberekening van de balk, en niet de aanroepen van `kaart_mat`. Nieuwe test `test_stilstand_verandert_niets_aan_de_balk`: tien frames niets doen → nul tellingen en geen verschoven kost; hulplijn en tweede regel wórden gehoord; daarna weer stil, óók met de hulplijn op het scherm; kaart weg → ruimte terug. **Suite 474 goed, 0 fout** (was 473, +1 test), `DH_TEST_FILTER=test_balk` 20/0, `DH_TEST_FILTER=was` 21/0, `core/sommen.gd` byte-gelijk. Geen beeld: er is niets aan te zien — zelfde papier, zelfde balk, minder metingen. **Niet gepusht.**
- 2026-09-20 04:40 (nacht, lokaal model) — **S5 — een misser: de strook gaat op slot en dezelfde vraag komt terug.** Bestanden: `autoload/ui.gd`, `ui/keuzestrook.gd`, `ui/teksten.gd`, `tests/test_ui.gd`, `PLAN.md`, `HOTEL.md`, `.fanout/specs/godot/games-b.md` — plus **één bestand buiten de set: `games/was/test_was.gd`**, hieronder uitgelegd. `Ui.misser(kaart, dier)` is de publieke ingang en doet de drie dingen uit §4: `World.pose(dier, "sip", SIP_TIKKEN = 22)`, een wolkje `🔄 Nog een keer` aan het dier (`UiTekst.MIS_ICOON`/`MIS_ZIN`), en `UiKeuzes.slot(true)` — elke knop `disabled` én `modulate.a = 0.45`. De pauze hangt aan de kaart: `Ui.misser` is een co-routine die `create_timer(MIS_PAUZE = 1.2)` afwacht en dan `_mis_vrij(kaart)` aanroept, dat alleen iets dóét als `kaart.pauze` nog `true` is; `Kaart.klaar()` en `Kaart.weg()` zetten `pauze` zelf op `false` en ruimen het wolkje op, dus een spel dat midden in de pauze stopt of afvinkt laat niets achter en de lopende timer wist geen ✓ die het spel intussen zette. De grendel zit op twee plaatsen: `disabled` houdt de vinger weg, en `UiKeuzes.op_slot` houdt de `pressed`-lambda tegen vóór `Snd.tik()`, want `emit_signal("pressed")` negeert `disabled` — zonder die vlag zou een script (en een test) dwars door het slot heen kunnen. `_getal_keuzes` roept `misser()` vóór `roep(k.on_ok, …)` en `on_ok` wordt onveranderd aangeroepen, zodat elk spel zijn eigen `missers`, `Snd.zacht()` en hulpladder houdt. **Drie afwijkingen van de letter van §4, expliciet genoemd:** (1) het plan vroeg `😢 Nog een keer`; U+1F622 zit **niet** in `fonts/Emoji.ttf` (de subset van 112 tekens die dit build meelevert) en tekent tofu — `test_alle_schermteksten_hebben_een_glyph` pakte dat onmiddellijk. 🔄 (U+1F504) zit er wél in en is in dit spel al het huispictogram voor "opnieuw" (`voerkar/ICO_OPNIEUW`, tobbe's "opnieuw", bedden' "terug"), dus plaatje en woord zeggen hetzelfde; het verdrietige blijft in het dier zelf zitten. (2) `games/was/test_was.gd` is niet van S5 maar moest mee: de was tikt band 4 en 5 hun nummerstrook **achtereenvolgens zonder te wachten**, en precies dát blokkeert S5 — de tweede en derde tik werden geslokt en de hulpladder bleef bij één misser staan. De test krijgt `_wacht_mispauze()` (Ui.MIS_PAUZE + 250 ms) tussen de tikken, zodat hij tikt zoals een kind met een echte vinger; het spel zelf is niet aangeraakt. Zonder die aanpassing was de suite rood en regel (b) staat boven regel (d). (3) Het plan zei "`DH_TEST_FILTER=games` groen zonder wijziging in de spellen" — dat klopt voor de spellen, maar niet voor de *tests* van de spellen; dit is het geval dat het plan niet voorzien had. **Wat er nu nog níét te zien is, en waarom:** geen enkel spel geeft `dier` door aan `somkaart`, en geen enkele `obj` in de tien spellen is een dier-id, dus de `World.dier(obj)`-terugval uit §4 stap 1 vindt niets en het sip-dier + wolkje verschijnen vandaag alleen waar een kaart expliciet `"dier"` meekrijgt (nu alleen in `tests/test_ui.gd`). Het slot zelf wél, overal. Voorstel voor de lead: geef in `V2`/`V5` (voerkar), `N9` (bedden), `K1` (sleutels) en `K3` (wekker) het dier van de beurt door via `o["dier"]`, dan klopt de wens van de eigenaar ook in het beeld; §4 stap 4 schrijft voor die spellen alleen voor dat ze `Ui.misser` zelf aanroepen, niet dat ze hun dier noemen. Bewijs: `DH_TEST_FILTER=test_ui` **22 goed, 0 fout** (zes nieuwe: `test_een_misser_maakt_het_dier_even_sip`, `test_de_strook_gaat_even_op_slot`, `test_dezelfde_keuzes_komen_terug`, `test_een_misser_kost_niets`, `test_zonder_dier_alleen_de_pauze`, `test_sluiten_tijdens_de_pauze_lekt_niets`), `DH_TEST_FILTER=was` **21 goed, 0 fout**, `DH_TEST_FILTER=games` **209 goed, 0 fout**, volle suite **480 goed, 0 fout** (was 474), nul `SCRIPT ERROR`-regels, `core/sommen.gd` byte-gelijk. Beeld: tobbe-tijd in de tuin, band 4, "helft van 8 = [ ]" met de vier knoppen 0/4/5/8 en daarna na een tik op de 0 — de strook licht en grijs, de knoppen uit; tablet 1024×768@2 en telefoon 360×740@3. **Niet gepusht** — de lead controleert.
- 2026-09-20 04:40 — **open punt voor de lead uit `S5`** (boven): geen enkel spel geeft `o["dier"]` door, dus het sip-dier is in het spel nog nergens te zien. Niet opgelost in S5 omdat geen enkel spelbestand in de bestandsset van S5 staat.
- 2026-09-20 06:40 (ochtend, lokaal model) — **B3 — het anker `"balk"` telt niet meer tegen de plafond; de drie tests uit de acceptatie zijn herschreven.** Bestanden: `autoload/hits.gd`, `tests/test_hits.gd` (+ `PLAN.md`). **Stap 1, 2 en 5 van §4 stonden er al**, uit B2: `_op_van` test `"balk"` vóór `kleef_aan` (`hits.gd:602-606`), `_kies_plek` heeft de tak die `Ui.balk_plek(kind, maat)` aanroept en terugvalt op `"midden"`/`"onder"` als die leeg is (`:639-651`), `"balk"` zit in de overslaglijst van `_blijf_staan`, en `Ui.somkaart()` is onveranderd. **De enige nieuwe code is stap 3**: de cull-lus telde `MAX_PER_KAMER` over *elke* spot in de kamer; nu telt een spot met anker `"balk"` niet mee, want de balk beperkt zichzelf al — één kaart en één strook, en `Ui.balk_bepaal()` laat beide los als ze niet passen. De oude lus was `for i in lijstje.size(): if i >= MAX_PER_KAMER: lijstje[i].zichtbaar = false`, nu een lus met `geteld` die balk-spots overslaat. Nieuwe test `test_balkplekken_tellen_niet_tegen_de_plafond`: 17 gewone knoppen (één boven het plafond) plus een gedokte kaart met zijn strook → **16 gewone over, beide balkplekken intact**; de kamer verliest een eigen knop, geen balkplek. **Herschreven zoals §4 het vroeg:** `test_keuzestrook_kleeft_onder_de_kaart` is `test_strook_staat_onder_de_som_in_de_balk` — anker `"balk"` voor kaart én strook, beide rects volledig binnen `Ui.balk_rect()` (`.encloses`), en de volgorde er nu wél in: vorm `hoog` → `kaart.rect.end.y <= strook.rect.position.y`, vorm `laag` → `kaart.rect.end.x <= strook.rect.position.x`. De oude test aanvaardde met `op == "kleef"` stil "strook boven de kaart"; die weg is dicht. `test_antwoordstrook_kleeft_aan_de_kaart` houdt álle andere eisen (strook bestaat, past in het kader, vier knoppen van ≥ 44 px) en laat alleen de `Hits.KLEEF`-afstand los — op het papier is die afstand `BALK_GAT`, niet `KLEEF`. `test_kaart_laat_de_balie_vrij` is omgegooid van "geen enkele `midden`-kaart raakt de balie" naar de drie dingen die er nu echt staan: de balk raakt de balie nooit (`intersectie-oppervlakte == 0`), de winnende kaart (`Ui.balk_kaart()`, hier `ci_som`) staat volledig in het papier, en de tweede kaart (`rek_som`) staat buiten het papier, niet op de balie, en bij haar eigen voorwerp. `_keur` (geen overlap, geen dekking, `krap == false`) geldt in alle elf kaders. **Vier dingen afwijkend van de letter van §4, hier genoemd (regel (e)):** (1) **stap 4 noemt `UiRekenbalk.balk_plek(kind, maat)`; die methode staat niet daar maar op `Ui`** (`ui.gd:269`) — dat was al de B2-beslissing ("Ui owns the geometry") en is gepusht in `a85504d`. `ui/rekenbalk.gd` is een 96-regelige Control die alleen het papier tekent, heeft geen geometrie en hoefde niets te veranderen, dus dat bestand staat **niet** in deze commit terwijl §4 het in de set noemt. (2) De **absolute offsets** uit §4 (kaart op `balk.y + 8`, strook op `kaart.end.y + 8`; laag: kaart op `balk.x + 12`, strook tegen `balk.end.x − 12`, beide verticaal gecentreerd) zijn niet wat B2 bouwde: `BALK_LUCHT = 2.0`, onder-geankerd, en in `laag` staat de kaart links en de strook rechts. De **relaties** die de acceptatie eist gelden wél; de pixelgetallen niet. Wie de §4-offsets wil, moet dat zeggen — het is een beslissing, geen bug. (3) **Het `laag`-formaat dockt in géén van de tien echte frames**: op 740×360 is `balk_ruimte()` 29 en `balk_nodig()` oneindig, dus de acceptatieclausule over `laag` zou nooit draaien. De nieuwe test voert daarom een elfde kader in, `Vector2(700, 430)` — de laagste hoogte waarin `laag` wél dokt (papier + kaart en strook naast elkaar, plus `WERELD_BANDEN` = 5 banden over). Zonder dat kader was dat deel van de eis toevallig ongetest. De test faalt hard als er geen enkel `laag`-kader doorheen komt. (4) *"De tweede bij haar voorwerp"* is gemeten als "de kaart dekt de kolom van haar eigen mik-punt (`_laatste[id]["mik"]`) en ligt boven het papier", **niet** als een vast aantal banden: het mik-punt van de balie zélf ligt binnen de balk, dus de verliezer wordt een hele balkhoogte omhoog gedrukt (208 px op 1024×768, kaders van 3 banden = 156 px). Een banden-grens was hier dus onhaalbaar; de kolom is wat "bij haar voorwerp" in dit spel betekent. **Bewijs dat de spellen ongemoeid blijven:** `DH_TEST_FILTER=wekker` **23 goed, 0 fout** en `DH_TEST_FILTER=hinkel` **19 goed, 0 fout**, met `git diff --name-only -- godot/dierenhotel/games/` leeg — geen van beide spellen, en geen van beide tests, is aangeraakt. **Volle suite 481 goed, 0 fout** (was 480, +1 nieuwe test), nul `SCRIPT ERROR`-regels, `core/sommen.gd` byte-gelijk, export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer receptie --tik bel` op tablet 1024×768@2 (`/tmp/b3tab/ipad-land-3-bel.png`), telefoon 360×740@3 (`/tmp/b3tel/telefoon-3-bel.png`) en laag-land 740×360@3 (`/tmp/b3laag/laag-land-3-bel.png`) — op tablet en telefoon hangt de check-in-kaart in het papier met de strook eronder, op laag-land laat de balk los (ruimte 29 < nodig) en staat de kaart ouderwets boven in de kamer met de strook eronder, precies zoals B2 dat voor dat frame vaststelde. **Observatie voor de lead, buiten deze taak:** elke kiek logt `WARNING: 'res://scenes/main.tscn': In external resource #3, invalid UID: 'uid://baoutqwedycyj' - using text path instead: 'res://ui/rekenbalk.gd'`. Hij overleeft een verse `tools/import.sh` + `tools/export.sh`. De bron in `main.tscn` verwijst `rekenbalk.gd` bij **pad** (geen `uid=`), `ui/rekenbalk.gd.uid` op `main` is `uid://bvw3ys0msfnx1`, en `uid://baoutqwedycyj` is de uid die hetzelfde bestand had in de B2-worktree `.claude/worktrees/wf_d8939a2e-e84-1` — de exporter lijkt een oude uid→pad-afbeelding mee te nemen. Het is een warning met werkende terugval: het script laadt, het spel draait, er is niets kapot. `scenes/main.tscn` staat niet in de B3-set, dus niet aangeraakt. **Niet gepusht** — de lead controleert.

- 2026-09-20 07:45 (ochtend, lokaal model) — **V5 — het bakje blijft even vol en de knop liegt niet meer.** Bestanden: `autoload/world.gd`, `scenes/main.gd`, `tests/test_world.gd` (+ `PLAN.md`). **De kern van de bug:** `_eet()` poortte op `_tikken % 5 == 0`, een teller op de *wereld*. Elke etende gast nam dus op precies dezelfde tik één niveau mee: vier gasten aan één bak hapten een vol bakje in vijf tikken ≈ 0,33 s leeg, terwijl het 😋-wolkje van 2,6 s nog boven hen stond en de bakknop in die hele tijd "🍪 Vol" bleef zeggen. **De fix:** de kauw telt niet per dier en niet per wereld, maar **per bakje**. `Dier.kauw` blijft over en bestuurt alleen de kaak (`hap1`/`hap2`, nu over de eigen teller, zodat twee gasten niet meer in elkaars kielzog happen); `_bak_kauw["kamer|slot"]` is de nieuwe teller die `_bakjes_kauwen()` één keer per denk-tik optrekt zolang er iemand aan dat bakje staat, en bij `KAUW_PER_NIVEAU = 24` tikken (≈ 1,6 s) gaat er één niveau af. Vier niveaus kosten dan ±6,4 s, **ongeacht of er één gast eet of vier** — de zin uit §4 die met een per-dier-teller alsnog niet zou kloppen (mijn eerste opzet, met de teller op het dier, maakte een bakje van vier gasten in 1,6 s leeg; de foto hieronder bewees het). `zet_bak()` wist die teller bij elke echte verandering, dus "sinds deze bak voor het laatst bewoog" is wat er staat, en elk niveau is ook over testgrenzen heen precies 24 kauwtikken. Buiten beeld blijft alles oud: `_grof_tik()` maakt de bak in één keer leeg (world.md §2.9). **Signaal:** `bak_veranderd(kamer, slot)` op `World`, maar `zet_bak()` zendt hem alleen als het niveau écht schuift — 4 over 4 schrijven is geen nieuws en mag dus ook geen repaint wakker maken, en 0 over een onbekende sleutel ook niet (`bak_stand` leest die als 0). **Afwijking van de letter van §4, hier genoemd (regel (e)):** de taak zegt *"een handler die `Hotel.render()` call_deferred aanroept"*; die handler coalesst — `_bak_gewijzigd` zet een vlag en schiet `_schildert_bak()` één keer per frame af, die de vlag wist en dan tekent. Vier dieren die samen beginnen eten vuren alle vier op dezelfde tik, en `Hotel.render()` herbouwt elke hotelknop van voren; zonder die vlag waren dat vier volledige herbouwen in één frame op een tablet. Verder houdt de handler zich aan de taak: alleen de kamer die in beeld wordt getekend. **Vier nieuwe tests** (`tests/test_world.gd`): bakje op 4 met twee etende dieren is na 20 tikken nog ≥ 3 en na 100 tikken 0; `bak_veranderd` vuurt precies één keer per echte verandering (0 over 0: stil, 0→4: één, 4→4: stil, 4→3: twee, `9` klemt naar 4: drie) en noemt `kamer1|bak`; een handler die vanuit het signaal midden in de eetlus `Hotel.render()` aanroept gooit niets om en eet de bak netjes naar 0; en `test_eters_veranderen_het_kauwtempo_niet` meet voor 1 én voor 4 eters dezelfde 96 tikken tot de bak leeg is. `_na_afloop()` ruimt nu ook `t4`/`t5` op. **Suite 485 goed, 0 fout** (was 481, +4), nul `SCRIPT ERROR`-regels, `=test_world` 30/0, `=voerkar` 16/0, `=test_ui` 22/0; export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer keuken --band 4` door de echte voerkar heen — 40 koekjes over 4 gasten, `Klaar`, de kar via de gang naar kamer 1, daar het bakje aangeleverd. Direct na de levering (`/tmp/v5foto/ipad-land-h-geleverd.png`) staat het 😋-wolkje "10 koekjes elk" boven de kamer, zegt de bakknop 🍪 Vol en ligt er bruin voer in het bakje. **Drie seconden later, terug in de kale hotel-shell** (`/tmp/v5foto/ipad-land-j-hotel-3s.png`) ligt er nog steeds zichtbaar voer in de bak en zegt de knop nog steeds 🍪 Vol — precies wat de acceptatie vroeg. Twee seconden vóór de levering (`/tmp/v5d/ipad-land-f-kamer1.png`) stond er nog "🍽 Leeg" met het ⚠-bandje, dus het verschil is op de beelden echt te zien. **Observatie voor `V2`, buiten deze taak:** tijdens het rondbrengen leent het spel de bakknop van de hotel-shell (`_leen_doelen`, `spel.gd:870`) en na een levering aan die kamer wordt hij niet opnieuw geleend, maar ook niet teruggegeven — er staat nergens een `geef_terug`-aanroep in `games/voerkar/spel.gd`. De bakknop van de geleverde kamer verdwijnt daardoor tot het spel uit is (`/tmp/v5foto/ipad-land-i-na-3s.png`, waar de bak nog vol ligt maar geen knop heeft). Bestaand gedrag van de voerkar, niet van V5, en `games/voerkar/spel.gd` staat niet in deze bestandsset.

- 2026-09-20 12:49 (middag, lokaal model) — **PAUZE op verzoek van de eigenaar; de V2-vraag is beantwoord, V2 zelf is nog niet begonnen.** **Opdracht voor de volgende sessie: batch 3, taak `V2`.** **Wat af is en gecommit (niet gepusht):** `N8` `553ddee` · B2-nazorg `ed9bb40` · `S5` `737fb77` · `B3` `506f826` · `V5` `1a8a5e2`; de batchtabel staat op **4 / 9** en elk van die taken heeft `[x]` met datum en model. **De V2-beslissing van de eigenaar is "doorduwen"** — optie A op de vraag wat `stap = "som"` in V2 moet doen nu de vul-fase weg is en V3 de echte som-poort nog niet bouwt. In code: `start()` zet bij een verse start `K["op_kar"] = K["T"]` en schiet meteen door naar `K["stap"] = "duwen"`, zodat de hele lus (keuken → gang → kamer → bakje vullen → terug) speelbaar blijft en er geen dood scherm ontstaat. V3 vervangt deze route door de echte poort (`_som_kaart`/`_op_som`, waar `op_kar = per * n` uit het antwoord valt). **Zeg deze tussentijdse route expliciet in de commitboodschap van V2** — het is het enige stukje dat V2 méér doet dan slopen. **Wat er van de slooplijst uit §4 bij zit (alles is gelezen, geen regel geschreven):** te schrappen zijn `MODEL_BAKJE`, `VAKBREUK`, `POTBREUK`, `BAK_DX`/`BAK_DZ`, `MINI_VLAK`, de vlaggen `_mini`/`_krap`/`_kort`/`_laatste_vorm`, `_bakje_model`, `bak_plek`, `_bak_id`, `_bakje_zet`, `_bakjes_weg`, `_bakjes_neer`, `_bakje_hotspot`, `_snap_hoogte`, `_chip`, `_chip_hoog`, `_naam_kort`, `_meet_kader`, `_zak_neer`, `_tik_zak`, `_knoppen_neer`, `verplaats`, `_val_vak`, `_kruimels`, `opnieuw`, `_tik_opnieuw`, `zet_hand`, `_kies_hand`, `volgende_hand`, `_kleur_hand`, en de `keuzes`-blok in `_kaart_neer` (de pak-strook). **Els blijft:** `_tik_els`/`hulp()`/`ICO_ELS`/`T_ELS*` overleven, maar de knop moet uit `_knoppen_neer` verhuizen naar de nieuwe tekenlaag, want die functie zelf gaat weg. **Een afwijking die V2 met zich meebreft en die in de boodschap moet:** `check()` en `_fonkel_bakjes()` hangen aan de vul-fase (`K["zak"]`, `K["vak"]`, `bak_plek`) en zijn met de nieuwe `K`-vorm leeg — er is niets meer te controleren. §4 schrijft voor dat `check()` in **V3** verdwijnt; in V2 sneuvelt hij noodgedwongen al, want de data waarop hij controleert bestaat dan niet meer. `_fonkel_bakjes()` is het slot van `check()` en gaat mee. **De nieuwe `K`-vorm** is `{T, per, rest, op_kar, pot, stap, geleverd, missers, spook, t0}`; `_herstel()` telt de oude velden om (`vol: true` → `stap = "duwen"`, geen `vol` → `stap = "som"`, `zak`/`vak` → `op_kar`). **Twee open punten die de volgende sessie niet mag vergeten:** (1) uit `S5` — geen enkel spel geeft `o["dier"]` door, dus het sip-dier is in het spel nog nergens te zien; `V2` is de plek waar dat voor de voerkar kan. (2) uit `B3` — elke kiek logt `WARNING: 'res://scenes/main.tscn': In external resource #3, invalid UID: 'uid://baoutqwedycyj'`; de bron is een oude uid→pad-afbeelding van `ui/rekenbalk.gd`, de fix raakt `scenes/main.tscn` en staat in geen enkele bestandsset. **Nada te doen na V2:** `N9` → `K1` → `K3` → `R2`, dan batch 4. Alles op `main`, één taak per commit, **niet pushen**.
- 2026-09-20 20:15 (avond, lokaal model) — **V2 — de keukenvloer is leeg; de kar vertrekt vol.** Bestanden: `games/voerkar/spel.gd`, `games/voerkar/test_voerkar.gd`, `PLAN.md`. **De sloop is compleet:** alle namen uit de slooplijst van §4 zijn uit de bron — `MODEL_BAKJE`, `VAKBREUK`, `POTBREUK`, `BAK_DX/DZ`, `MINI_VLAK`, de vlaggen `_mini`/`_krap`/`_kort`/`_laatste_vorm`, de hele bakjes- en zaklaag, `verplaats`, `opnieuw`, de schep-helpers, de meet-/chip-helpers en de pak-strook in `_kaart_neer`. `deelnemers()` telt nu alleen gasten met een bed in een kamer met een `bak`-slot. De nieuwe `K`-vorm `{T, per, rest, op_kar, pot, stap, geleverd, missers, spook, t0}` is neergezet met `_herstel()`: `vol: true` → `duwen`, geen `vol` → `som`, `zak`/`vak` → `op_kar`; een oude save gaat dus zonder verlies over. **De tussentijdse route (eigenaarsbeslissing "doorduwen"):** `start()` zet bij een verse beurt `op_kar = T` en `stap = "duwen"`, zodat de hele lus speelbaar blijft tot V3 de echte som-poort bouwt — dit is het enige stukje dat méér doet dan slopen. **Els blijft:** `_tik_els`/`hulp()` overleven en haar knop hangt nu bij het rondje mee (tekenlaag `_rondje`). **Afwijkingen van de letter van §4, expliciet:** (1) `check()` en `_fonkel_bakjes()` sneuvelen al in V2 (noodgedwongen — de data waarop ze controleerden bestaat niet meer; §4 plande dit voor V3). (2) De duw-wolk van 55 px is op 740×360 twee banden hoog en er ligt geen vrij blok van vijf kolommen bij de kar — de keuken deuren en de kar zelf kruisen elke rakende band — dus het icoon van `vk_duw` krijgt `icoon` (23) in plaats van `icoon_wolk` (25): de bubbel wordt ±52 hoog, één band, en landt schoon bovenin (`krap: false`, `op: boven`). Zonder die maat dekt hij het kozijn van de tuin (60 px²) en faalt de 0 %-regel. Dit was een bestaande layoutbug die de oude test nooit raakte (die testte het drukste vullen-beeld, niet het rondje). **S5's open punt is gedaan:** de opdrachtkaart draagt `o["dier"]` (het dier van de beurt), meegeven in `_kaart_neer`, zodat V3's misserweg sip kan worden. **Beweis:** `DH_TEST_FILTER=voerkar` 17 goed, 0 fout; volle suite **486 goed, 0 fout** (was 485), nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk; export zonder fouten. Beeld: `--kamer keuken --band 4 --tik spel_voerkar` (`/tmp/v2/ipad-land-3-spel_voerkar.png`) — geen bakje op de vloer, geen zak, geen pak-knoppen; in het spel-log zijn het maar **2** eigen knoppen (`karhot` + `vk_duw`) waar het oude beeld er tien-plus had. De bekende B3-UID-waarschuwing (`main.tscn` → `rekenbalk.gd`) leeft nog, buiten deze set. **Niet gepusht** — de lead controleert.
- 2026-09-20 22:05 (avond, lokaal model) — **N9 — Bedden begint met een echte vraag; Wolkje legt één bedje.** Bestanden: `games/bedden/spel.gd`, `games/bedden/test_bedden.gd`, `PLAN.md`. **De nieuwe eerste fase `"vraag"`** staat in `ctx.data()["fase"]`: de kaart toont `%d × %d =` (rijen × perRij) mét `goed: _doel()`, regel `Hoeveel bedden heb je nodig?` (5/28), pictogram 🛏 en vier knoppen `🛏 <n>` via de `goed`-strook van `Ui` (zaad = kaart + dag, dus dezelfde vier bij elke poging en na een herlaad). Fout = `Snd.zacht()` + de telladder `tel mee: 2 … 4 … 6.` als hulpregel op de kaart; de centrale `Ui.misser()` geeft de sip-pauze en dezelfde keuzes — nooit een kruis, nooit een stap terug, en het telt **niet** mee als leg-misser. Goed = `klaar()` (groen ✓) en fase `"leggen"`. **In `"vraag"` bestaan de strookjes, de kist, 🔄 en 🐾 niet** — en het gastwolkje blijft weg, want zijn getal zou het antwoord verklappen. In `"leggen"` verschijnen ze en houdt de kaart haar bestaande `Leg %s van %s` met het meetellende antwoordvak. **Wolkje (`hulp()`)** legt nu **één** bedje via `leg_in(rr)` in plaats van een hele rij; spookbedjes en spookcijfer blijven, titel `nog een bedje erbij`. **De deur wordt niet meer geleend** in `start()` — 🐾 `bd_klaar` is de enige klaar-controle; weglopen is weglopen en de beurt staat in `ctx.data()` (eigenaarsbeslissing V3, §6: ja). De fase overleeft een herlaad: `"vraag"` en `"leggen"` staan in de save, oude saves met `"leg"` hervatten gewoon in de leg-fase. **De S5-open-punt is hier gedaan:** de vraagkaart draagt `o["dier"]` (de eerste gast zonder bed), dus de sip-pauze is nu ook in bedden te zien. **Beweis:** `DH_TEST_FILTER=bedden` **22 goed, 0 fout** (18 bestaande, aangepast aan de vraagfase, + 4 nieuwe: `test_het_spel_begint_met_een_vraag`, `test_wolkje_legt_een_bedje`, `test_twee_missers_en_hulp_geven_geen_ster`, `test_de_deur_verlaat_de_kamer`); volle suite **490 goed, 0 fout** (was 486), nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk; export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer kamer1 --band 4 --tik spel_bedden` (`/tmp/n9/ipad-land-3-spel_bedden.png`) — het eerste beeld is de vraagkaart met vier knoppen; het probe-log toont alleen `bd_som` + `bd_som_keuzes` in de balk, `krap=0`, dekking 0. **Afwijking van de letter van §4:** de taaktekst schreef `_leg_in(rr)`; de echte methode heet `leg_in(r)` — dezelfde aanroep. **Niet gepusht** — de lead controleert.

- 2026-09-21 11:45 (ochtend, lokaal model) — **K1 — Sleutels: eerst het getal kiezen, dan pas ophangen.** Bestanden: `games/sleutels/spel.gd`, `games/sleutels/test_sleutels.gd`, `autoload/hits.gd` *(buiten de set, eigenaarsbeslissing — zie hieronder)*, `PLAN.md`. **De beurt is er twee.** `_p["stap"]` (`"reken"`/`"hang"`) en `_p["gekozen"]` staan in `ctx.data()` en overleven een herlaad. In `reken` draagt de sleutel **geen** getal: de bron krijgt `"aantal": 0` mee, dus `UiBron._get_drag_data` geeft geen lading en er is niets te slepen; de kaart vraagt `🔑 Welk nummer mist er?` met vier getalknoppen uit `goed` (zaad = kaart + dag, dus dezelfde vier bij elke poging). Goed → `Snd.munt()` + `ja()`, `Ui.bron_van("sl_key").zet(n, 1)`, stap `"hang"`. Fout → `Snd.zacht()`, een misser die niets kost, dezelfde vier keuzes terug. Het actieve gat is het **meest linkse lege haakje** en gloeit tijdens `reken`; in de somregel staat het actieve gat als `__` en een later gat als `?`. In `hang` gloeien alle lege haakjes en werkt `_hang(i)` via tik én sleep. **B2, B3 en B4 zijn gerepareerd:** `_licht = -1` zodra `_p["nu"]` naar een ander bord gaat; `_buren_van` rekent over het gat van de **sleutel** (`s["haak"]`), niet over het aangewezen haakje; buurlijn en Els' tip staan samen op één hulpregel met `"  · " ertussen`. De hulpregel is **altijd** gevuld via één `_hulp_tekst()`. De twee lege `Ui.spreek()`-aanroepen zijn vervangen door wolkjes van 1,6 s (`sl_wijs`, `sl_lees`). `_meld_rij()` meldt nu ook `stap`, `gekozen`, `gat`, `sl_key`, `sl_tag` en `sl_els`. **Vier afwijkingen van de letter van §4, expliciet:** (1) De taak zegt *"bij N ≤ 4 is er precies één gat"*; gemeten over de bevroren kern heeft N=2 één gat, N=3 één gat en **N=4 twee gaten**. De test legt vast wat er wél altijd geldt: `gaten == aantal sleutels` voor N=2..4, en `gaten == 1` voor N ≤ 3. (2) Extra const `VRAAG_MIST_KAMER` voor de `kamers`-variant; de taak noemde die tekst niet. (3) `VRAAG_WACHT` staat op de kaart als `regel2`, niet als staand wolkje — als wolkje duwde die lange tekst op een smal kader tegen de naamplaat van de gast aan. (4) In de `reken`-stap op een laag kader (< 450 px) laat de kaart de somregel weg: die is daar een dubbeling van wat op het bord zelf staat, en zonder die regel is de kaart laag genoeg om onder het sleutelbord te passen. **De ingreep in `autoload/hits.gd` (buiten de K1-bestandsset, door de eigenaar gekozen op 2026-09-21):** op liggende telefoon (kader 558×289) waren onder de kaart, boven de kaart én de kadervoet alle drie door de kaart zelf bezet, zodat de strook met vier knoppen `krap` werd. De kleeflijst in `_kies_plek` krijgt daarom **twee extra kandidaten laatst in de volgorde**: de linker- en rechterzijkant van de kaart, op de hoogte van de kaart zelf. Laatst in de lijst betekent dat élke opstelling die vandaag werkt ongemoeid blijft — de drie bestaande kandidaten winnen nog steeds telkens. **Wat er mis was met de eerste meting, en de les ervan:** `_kaart_maat()` mat de kaart met `get_combined_minimum_size()`. Een sommenkaart met een **zichtbare hulpregel** antwoordt daarop de hoogte die zijn wrap-regel op de *laatst gelegde* breedte nodig heeft — gemeten 659 units voor één zin van vier woorden, precies de val die `hits.gd` al beschrijft. `Ui.kaart_mat()` meet de hulpregel op de breedte waarop de kaart écht getekend wordt, en dat is wat `Hits._maat_van()` ook doet. Nu plant het spel op dezelfde maat als die de knoppenlaag gebruikt; daardoor ging 1024×768 terug van `stapel` naar `gewoon`. **Beweis:** `DH_TEST_FILTER=sleutels` **22 goed, 0 fout** (15 bestaande + 7 nieuwe: de sleutel draagt geen getal vóór het antwoord, een fout antwoord straft niet en laat dezelfde vier keuzes staan, de keuzes blijven binnen de band, de stap overleeft een herlaad, de hulpregel is altijd geregen, bij vier gasten precies één gat per sleutel, en elke getekende regel past in het budget); volle suite **497 goed, 0 fout** (was 490), nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk; export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer receptie --band 4 --tik spel_sleutels` op tablet (`tmp/k1/ipad-land-3-spel_sleutels.png`) en telefoon (`tmp/k1t/telefoon-3-spel_sleutels.png`), beide `KIEK OK`, `krap=0`, `dekking_max=0.00`, `stap=reken`, `sl_key=1`, `sl_tag=0` (in de reken-stap is er geen gastwolkje — de kaart vraagt, de gast wacht). **Niet gepusht** — de lead controleert.
- 2026-09-21 13:30 (middag, lokaal model) — **K3 — Wekker: de hulpladder klopt en de wijzerplaat wordt gereserveerd.** Bestanden: `games/wekker/spel.gd`, `games/wekker/test_wekker.gd`, `PLAN.md` — binnen de set, geen vreemde bestanden. **De ladder is er drie.** `_nieuwe_beurt` begint met `b["hulp"] = T_HULP_BEGIN` ("💛 draai tot de klok klopt") — de hulpregel is vanaf de eerste tik gevuld; `_lees_stand` vult een lege hulp uit een oude opslag bij, behalve bij `stap == "wakker"`. In `klaar_tik`: misser 1 → `T_HULP1` ("💛 draai nog wat verder"); misser 2 → `tel_ladder()` (met terugval op `T_HULP1` als de ladder leeg is); misser ≥ 3 → `T_HULP2` én pas dán de spookmarkeringen (`klok_params`: `missers >= 3`, was `>= 2`). `kies_tijd`-succes zet de hulp terug op `T_HULP_BEGIN`. **De balk doet nu het werk van de `_hang_kaart`-klem:** `som_balk()` geeft **altijd** `"nu: <tijd>"` (de `mis`-tak naar `""` is vervallen) en `zin_zet()` herhaalt de klokstand niet meer — de `mis`-tak is weg, de kaart zegt in elke stand dezelfde wens. `wk_plaat` van prio 13 naar **15**, zodat de wijzerplaat gereserveerd wordt vóórdat de kaart (prio 14) geplaatst wordt; de kiek bevestigt `wk_plaat` 88×88 met `dekking=0.00` en de kaart 183×140 ernaast, beide `krap=false`. **`tel_ladder()` is gerepareerd:** de bevroren kern geeft `"duur": dh if duur else 0` (`core/sommen.gd:773`), dus bij `duur == 0` telde `range(0, duur+1)` één uur op ("💛 7"). Nu: terugval op `posmod(doelU - u, 12)`, en staat de wijzer al op het goede uur (alleen de minuten missen) dan is de ladder leeg en valt de aanroeper terug op de gewone hulpregel. **Twee afwijkingen van de letter van de taak, expliciet:** (1) De taak geeft de telladder onbegrensd; een ladder van meer dan vijf uur past niet op één regel van de kaart (185 px, hulp ~161 px) en liet de kaart met 27 px groeien, wat de acceptatie (kaarthoogte gelijk ≤ 2 px) breekt. De ladder is daarom **afgekapt op vijf stappen**; langer wordt `"💛 nog %d uur"` (nieuwe const `T_HULP_LAD`). (2) De bindende tekst van games-b.md §2.10, "👻 de spookwijzers wijzen mee" (29 tekens), past niet op één regel van de kaart en groeide de kaart met 27 px bij de derde misser; de echte fix (kaart minimaal zo breed als de hulpregel) zit in `ui/kaart.gd`, dat in de set van `B4` staat. **Op verzoek van de eigenaar (2026-09-21) is de tekst ingekort** naar "👻 spoken wijzen mee" (20 tekens), zodat de hoogte over alle treden gelijk blijft; de spec mag hiervan op de hoogte worden gebracht. **Beweis:** `DH_TEST_FILTER=wekker` **24 goed, 0 fout** (nieuwe test `test_kaarthoogte_blijft_gelijk_over_de_stappen`: `zet`/`mis 1`/`mis 2`/`mis 3`/`duur`/`duur-mis`/`zet-na-duur` binnen 2 px, gemeten met `Ui.kaart_mat`; de bestaande ladder- en spooktests zijn omgezet naar de drie treden, `test_spookwijzers_pas_na_twee_echte_pogingen` → `..._pas_na_drie_echte_pogingen`); de eis dat `wk_plaat` in alle vier kaders ≥ 20×20 vrijhoudt wordt al gedekt door de bestaande `test_de_klok_is_te_zien_en_de_plaat_blijft_vrij`. Volle suite **498 goed, 0 fout** (was 497), nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk; export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer gang --band 4 --tik spel_wekker --uit tmp/k3` → `KIEK OK`, `stap=zet`, `wk_plaat` 88×88 `dekking=0.00`, kaart 183×140 `dekking=0.00`. **De "ná een misser"-shot kon niet met kiek.js gevangen worden:** kiek tikt alleen hotspots, niet de ✅-knop ín de strook, en een ingezaaide `mis`-opslag laat zich niet via `--url` doorgeven (kiek plakt zijn eigen `?opslag=` erachter); `tools/kiek.js` staat in de set van `R2` en is daarom niet aangeraakt. De mis-staten zijn bewezen door de tests. **Niet gepusht** — de lead controleert.
- 2026-09-21 15:15 (middag, lokaal model) — **R2 — De Speelzaal 🧸.** Bestanden: `autoload/rooms.gd`, `ui/plattegrond.gd`, `autoload/snd.gd`, `art/decor_speelzaal.gd` *(nieuw, + `.uid`)*, `art/decor.gd`, `tests/test_rooms.gd`, `tests/test_sfeer.gd`, `tests/test_plattegrond.gd`, `HOTEL.md`, `tools/kiek.js`, `PLAN.md` — plus **`tests/test_kamerbalk.gd` buiten de set** (afwijking (1) hieronder). **De kamer.** `w 114, d 100, wand 56, vloer "hout", loop 1.5`, lichtblauwe mat (20..94 × 18..82), zone `dans` (40..74 × 56..80), deur naar de receptie: speelzaal-kant `wand "z" at 57` → punt (63, 0), binnenstap (63, 8); receptie-kant `wand "x" at 108` → punt (0, 114), binnenstap (8, 114), bij de plant in de verre hoek. **Het decor** staat in het nieuwe `art/decor_speelzaal.gd` (`class_name ArtDecorSpeelzaal`, eigen `NAMEN` + `tabel()`, aangemeld in `ArtDecor._extra()`; de basis-`NAMEN` blijven 34 stuks): klimrek (16,14), ballenbak (96,14), blokkentoren (12,86), kussenhoek (100,86), muziekdoos (57,90), wimpel (30,50) — een layout die uit **meten** kwam (kopieer niet uit het hoofd): de eerste zetting gaf 17 dwaalplekken, deze geeft **20** met een minimum-afstand plek→decor van **18.0** (de eis ≥ 19 / ≥ 18 is damit exact gehaald, gemeten met `Rooms._bouw_vrij`/`_bouw_plekken` in een headless run). **De receptie verliest géén dwaalplekken:** 25 vóór en ná de nieuwe deur (de 14-ruit van de binnenstap dekt 206 vrije *cellen*, maar geen enkele van de 25 *loopplekken* — de claim "nul vrije vakjes" uit de taak gold de loopplekken en klopt daar; de celfreque is wél kleiner, gemeten). **Geluid:** `Snd.SFEER["speelzaal"] = "speeldoos"` + een `speeldoos`-tak in `sfeer_monster()`: C-E-G-E (880/1109/1319/1109 Hz sine, top 0.05) één noot per seconde over de 4-s-lus, met een zachte `_ruisband` eronder. **Plattegrond:** `"speelzaal": [1, 1]`. **Drie afwijkingen van de letter van de taak, expliciet:** (1) `tests/test_kamerbalk.gd` staat niet in de R2-set maar wél in de acceptatie als poort, en kon met tien chips (negen kamers + kaart) niet groen: de 295-unit-rail heeft dan de scroll-val nodig die `_meet_rail` zelf beschrijft ("moet de kamer na de volgende overleven"). De assertie "de negen chips van vandaag passen zonder te rollen" is daarmee ontwerpwijzig onwaar; vervangen door de bestaande hoogtecheck plus de bestaande rol-bereikbaarheidscheck (laatste chip rolt in beeld). Test-only, geen productiegedrag gewijzigd. (2) `test_de_plattegrond_is_een_plattegrond`: `deurparen()` 7 → **8** — de nieuwe deur ligt tussen twee buurcellen en wordt getekend; gegenereerd uit de run, niet overgetypt. (3) De taak zegt "maak `Kamer.matten` ongetypt"; de veldcode was al `var matten: Dictionary = {}` en is nu `var matten = {}` zoals voorgeschreven — `ArtVloer._matten()` slikt beide, niemand anders leest het veld getypt. **Beweis:** `DH_TEST_FILTER=test_rooms` 15/0, `=test_art` 23/0, `=test_sfeer` 3/0, `=test_plattegrond` 4/0, `=test_ui` 22/0, `=test_kamerbalk` 3/0; volle suite **498 goed, 0 fout** (het aantal testfuncties is gelijk gebleven — de R2-werkzaamheden zitten in bestaande tests en tabellen), nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk; export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer speelzaal --band 4 --uit tmp/r2` en `--viewport 360x740@3:telefoon --uit tmp/r2t`, beide `KIEK OK`, `hits=5 krap=0 dekking_max=0.00`; `--kamer receptie --uit tmp/r2r` toont de twee deurknoppen (`deur_receptie_gang` + `deur_receptie_speelzaal`) en de ongemoeide balie. **Niet gepusht** — de lead controleert.

- 2026-09-21 17:40 (avond, lokaal model) — **EIGENAARSWENS 2026-09-21 — Bedden: de kamer is leeg als de vraag valt.** Bestanden: `scenes/kamer.gd`, `autoload/world.gd`, `games/bedden/spel.gd`, `games/bedden/test_bedden.gd`, `tests/test_zzz_kamer_leeg.gd` *(nieuw, + `.uid`)*, `PLAN.md`. **De wens, letterlijk van de eigenaar:** "de kamer moet LEEG zijn op het moment dat de gast aankomt en de kaart vraagt `Hoeveel bedden heb je nodig?` — nu blijven de al gelegde bedden met slapende gasten zichtbaar in fase `vraag`", zodat het kind ze kon aflezen in plaats van te rekenen. **Het besluit is vooraf gevraagd en gegeven** (ask_user `ask_95f51bad96281277e5fc3bdb`): de **additieve render-haak**, geen ingreep in de data of in de bedden van andere spellen. **De haak.** `scenes/kamer.gd` onthoudt bij het bouwen welke nodes een bed zijn (`_bed_nodes`, geregistreerd in `_bouw_slot` op `slot["soort"] == "bed"`, geleegd in `bouw()`, en na élke herbouw opnieuw toegepast via `_zet_bedden(not _bedden_verborgen)`), en krijgt `verberg_bedden(aan)`: dat zet die nodes weg én roept `_ververs_dieren()` opnieuw, dat via `_slapende_gasten()` (de gasten met een niet-lege `bed` in deze kamer, uit `State.gast_in_bed`) ook de slaper die erin ligt verbergt en zijn naamplaat weghaalt. Een gast **zonder** bed blijft gewoon in beeld. `World.bedden_verberg(aan)` is de doorlaat ernaar, beschermd met `has_method` — geen kamer in beeld of een kale test-scene doet dus niets. **Het spel.** `_vraag_teken()` vraagt `bedden_verberg(true)` (de `_rustige_kamer()` die er al stond verbergt de bed-*knoppen*, nooit de modellen); `_op_vraag()` doet `bedden_verberg(false)` vóór `_teken()` zodra het antwoord klopt en de fase op `"leggen"` gaat; `stop()` doet het altijd, zodat het spel de kamer nooit verborgen achterlaat. Verbergen is render-only: `Rooms.slots`, de capaciteit en de bevroren kern zijn niet aangeraakt. **Waarom de kamertest níet bij `games/bedden/test_bedden.gd` staat** (afwijking van de opdracht, expliciet): dat is het **eerst** geladen testbestand van de suite (`res://games/...` sorteert vóór `res://tests/...`), en wie vandaaruit `res://scenes/kamer.gd` laadt zet diens hele preloadketen vast. Gemeten, niet geraden: **29 ObjectDB-instanties en 7 hulpbronnen "still in use at exit"** — `core/sommen.gd`, `core/jsgetal.gd`, `art/vorm.gd`, `art/effect.gd`, `scenes/wereldobject.gd`, `scenes/dier.gd`, `games/was/soorten.gd` — waar `tools/test.sh` (terecht) op afgaat; drie varianten (echte `.tscn`, kaal script, met/m zonder `Ui.naamplaten_leeg()`) gaven **exact dezelfde** 29/7, en met die test *uitgeschakeld* was de suite schoon (499/0, geen lek). Hetzelfde effect staat in de kop van dat bestand beschreven voor `extends Proef`: eerste positie lekken de kernscripts, laatste positie niet. Daarom staat de kamerkant in het nieuwe `tests/test_zzz_kamer_leeg.gd` (`extends Proef`, sorteert als allerlaatste), en de spelkant (`_TelKamer`-stub die de verzoekvolgorde optelt) wél bij de bedden-tests. **Beweis:** `DH_TEST_FILTER=test_zzz` **2 goed, 0 fout** (bed + slaper weg, gast zonder bed blijft, alles terug na `bedden_verberg(false)`; en zonder de vraag blijft álle gasten zichtbaar — wat elke andere kamer de hele dag ziet) en `DH_TEST_FILTER=bedden` **23 goed, 0 fout** (nieuw: `test_bedden_maken_de_kamer_leeg_rond_de_vraag` — weg bij `vraag`, terug na het goede antwoord, terug bij `stop()`); volle suite **501 goed, 0 fout** (was 498; +1 bedden, +2 kamer), nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk; export zonder fouten (18 bestanden, 41,5 MB). Beeld: `--kamer kamer1 --tik spel_bedden --uit tmp/bedden-kiek` → `KIEK OK`; in stap 3 (`vraag`) meldt de probe `bd_som` en `bd_som_keuzes` met `dekking=0.00` en **geen enkele `bed_kamer1`-knop meer**, terwijl stap 2 (vóór de tik) `bed_kamer1_bed2` nog rapporteert — de kamer is leeg precies zoals de eigenaar vroeg. **Niet gepusht** — de lead controleert.

- 2026-09-22 (lokaal model) — **B4 — Kaart en strook in balkvorm, met grote letters.** Bestanden: `ui/kaart.gd`, `ui/keuzestrook.gd`, `autoload/ui.gd`, `tests/test_balk.gd` (binnen de set) **plus `autoload/hits.gd` en `games/was/spel.gd` buiten de set** — eigenaarsbeslissing, zie de afwijkingen hieronder. **De kaartstand.** `ui/kaart.gd` kreeg `in_balk`: geen eigen paneel, geen liniatuur, maten uit `UiThema.balk_maten`, breedte = balkbreedte − 24, `regel2` vervalt in vorm `laag` (één `push_warning`); de krimp van de somlijn voor het verdwenen cijferpad is in de balk geschrapt; de lege liniatuur is weg (`som_label`/de hele `Rij`-HBox onzichtbaar als som én vak leeg zijn, `Ui.Kaart.som()` werkt mee). De inhoud die een spel ná het openen zet (`som`/`hulp`/`regel`/`regel2`/`zet`) staat nu in **live-velden** (`_hulp`/`_som`/`_regel`/`_regel2`/`_vak`) zodat een herbouw voor de balk of terug naar drijvend geen regel kwijtraakt. **De meting is de kern van deze taak.** Een wrappend `Label` rapporteert zijn hoogte op de breedte waarop het LAATST gelegd is; een vers gebouwde of net-zichtbare regel antwoordt één-woord-per-regel (een toren van 1225 units voor een kaart die 90 tekent). Daarom meet `inhoud_hoogte(breed)` de kaarthoogte **themagebaseerd** (`UiThema.wrap_hoogte` per zichtbare regel + de `Kolom`-separatie), nooit via `get_combined_minimum_size()`. `_meet_balk()` haalt de balkbreedte uit het **huidige** kader (niet uit `custom_minimum_size`, dat door `Hits.plaats` op de balkbreedte van het laatste kader vastgepind is), en de drijvende tak van `_bouw` zet `custom_minimum_size` terug op `ZERO` (de HEAD-geval: de breedte komt van de zin). **`autoload/ui.gd`:** `kaart_mat()` geeft een gedokte kaart `maat_balk()` terug en meet de drijvende kaart met `inhoud_hoogte`; `Ui.Kaart.zet()` routed via `zet_vak` (houdt het getal op het vak bij een herbouw). **Afwijking 1 — `autoload/hits.gd` (shared):** `_maat_van()` leidt élke sommenkaart door `Ui.kaart_mat()`, niet alleen de gedokte of de met hulpregel — een drijvende kaart zónder hulpregel viel anders terug op de Control-minimum-toren en werd buiten het kader geplaatst (bedden 558×289, hinkel, hits). De kleeflijst in `_kies_plek` krijgt **vier extra laatst-in-de-lijst-zijkantkandidaten** (links/rechts, boven- én onderuitgelijnd op de kaart): de eerlijke (ontpinde) breedte van een drijvende kaart is een paar units smaller dan de breedte die hij voorheen meedroeg, zodat de gecentreerde strook een tegel rakelings raakte (sleutels 740×360, een 4,6×11,7-sliver). Laatst in de lijst = élke opstelling die vandaag werkt blijft winnen. **Afwijking 2 — `games/was/spel.gd`:** `_kaart_maat()` meet met `Ui.kaart_mat()` in plaats van `get_combined_minimum_size()`, zodat de legenda-keuze kaart-vs-bubbel de eerlijke kaarthoogte ziet (anders koos de was de bubbel en werd de kaarttekst leeg). **Test-aanpassing:** `test_de_hulplijn_groeit_de_balk_mee` mat de balk-kost in plaats van de kaarthoogte; bij de brede B4-kaarten is één hulpregel korter dan die kost, dus de test meet nu de kaart tegen haar eigen hoogte vóór de hulp erbij kwam. **Beweis:** `DH_TEST_FILTER=balk` **20 goed, 0 fout**; `bedden` 23/0, `was` 21/0, `sleutels` 22/0, `kraam`/`meubels` 0 fout; volle suite **501 goed, 0 fout**, nul `SCRIPT ERROR`, `core/sommen.gd` byte-gelijk. Geen beeld: de balk staat al (B2); dit is de kaart/strook-in-de-balk-meting, geen nieuw uiterlijk. **Niet gepusht** — de lead controleert.

- 2026-09-22 (avond, Claude Opus) — **EIGENAARSWENS 2026-09-22 — Het papier: de letters staan óp de lijnen.** Bestanden: `ui/kaart.gd`, `ui/rekenbalk.gd`, `tests/test_balk.gd`. **De melding** (schermafbeelding van de inchecksom): "de tekst zit niet goed op de regels en gaat erdoorheen". **Oorzaak:** beide schilders trokken een vast raster van 22 — de zwevende kaart vanaf haar bovenrand, de balk vanaf de onderrand van het kader — terwijl een tekstrij 24 hoog is plus 4 kolomruimte; de lijnen liepen dwars door de letters, precies wat §3.1 al verbood ("lijnen op de regelhoogtes van de balk zelf"). **Nu:** `UiSomkaart.lijn_ys()` geeft voor elke getekende tekstrij (ook een afgebroken zin) de grondlijn + 0,5; de zwevende kaart tekent daar, de balk tekent op dezelfde hoogtes over de hele breedte en loopt met `lijn_afstand()` door tot onderaan, achter de knoppen. Opnieuw tekenen bij `sort_children` van `Kolom`/`Rij` en `item_rect_changed` van de kaart (gedokt: de balk). **De grondlijn is gemeten, niet aangenomen:** een Label-rij is zo hoog als de hele fontketen (Symbolen 15+9 bij maat 14) en de tekst staat daarin gecentreerd; `rijtop + ascent` (de eerste poging) lag 2–4 eenheden te hoog en sneed door de voet van elke letter terwijl een test met dezelfde formule groen bleef. De rijhoogte komt uit een substring van de gevormde tekst, zoals het Label zelf knipt: een losse `TextLine` telt in de webbouw het alleen geprobeerde Symbolen-font mee (24/14 i.p.v. 23/8 bij maat 22) — op de desktop is dat verschil er niet, dus alleen de webbouw liet het zien. **Bewijs:** drie nieuwe tests — `test_de_letters_staan_op_de_lijnen` (tien kaders × inchecksom, met hulpregel, vakkaart; gedokt én zwevend; grondlijn onafhankelijk via `TextParagraph`), `test_de_balk_lijnt_het_hele_papier`, `test_de_voet_staat_waar_de_webbouw_hem_tekent` (de uit de pixels gemeten voeten 17 / 25,5 zwevend en 26,5 / 41 in de balk). Tegenproef: met het oude raster 67× "lijn door de letters", met `rijtop + ascent` twee tests rood. Webbouw via `tools/speel.js` op 1024×768@2 (balk) en 1280×640@2 (zwevend): bij alle drie de rijen ligt de lijn één pixel onder de onderste inktrij. `DH_TEST_FILTER=balk` 26/0; volle `tools/test.sh` 504 goed, 0 fout, nul SCRIPT ERROR; `core/sommen.gd` onaangeroerd.

- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Grotere letters in de wereld, en vaste plekken voor deuren en spelingangen.** Bestanden: `autoload/hits.gd`, `autoload/hotel.gd`, `autoload/games.gd`, `autoload/ui.gd`, `ui/thema.gd`, `ui/kaart.gd`, `ui/hotknop.gd`, `ui/wolk.gd`, `ui/keuzestrook.gd`, `tests/test_balk.gd`, `games/sleutels/test_sleutels.gd`. **De melding:** "de tekst is er klein en minigames icoontjes en klik knopjes staan vaak nog op onduidelijke plekken die niet consistent zijn". **Eerst gemeten** (alle negen kamers, vier gasten, op het kader van de eigenaar: 1170×629, Firefox op een 4K-scherm): 10 van de 13 deurknoppen stonden recht onder hun deur, twee erboven (receptie → gang: taakkaartje `bord_2` pakte de vloer vóór de deur, prio 10 tegen 8; keuken → wasserij: het vak van de voerkar begint onder de drempel) en één erop (gang → keuken: een grote deur vooraan haalde de GROOT-toets die voor het prikbord bedoeld was). Vijf van de tien spelingangen hadden geen voorwerp: wie een verschuiving van meer dan 2 voxels heeft, vindt binnen `VLAK_NABIJ` niets en valt op het bandenrooster ("Bedden" midden in kamer 1). **Nu:** (1) een deurknop staat altijd vóór zijn deur — nooit eerst óp de deur (GROOT geldt niet voor deuren), anders op de voet van zijn eigen deur, pas dan erboven — en deuren kiezen met `prio` 11 vóór de zwevende taakkaartjes; op 1170×629 staan er nu 12 onder en 1 op zijn voet, op 326×558 geen enkele meer erboven (was 3). (2) Een spelingang met alleen een duwtje (|dx|+|dz| ≤ 8, `Games.DUWTJE`) noemt zijn voorwerp bij naam: "Bedden" hangt aan de speelmand en "Sleutels" aan het sleutelbord, zoals hun eigen tests al zeiden. (3) Nieuwe maat `wereld` in `UiThema.maten()`: knoplabels, bubbels, de zwevende antwoordstrook en de zin van een zwevende kaart op de basismaat (18) waar het kader ruim is (korte zijde ≥ 600: tablet, desktop), op telefoonkaders ongewijzigd `klein` — daar zijn de spelindelingen op die maten gesneden (bij 18 dekte de hinkelkaart haar eigen gast en de sleutelkaart haar bord). Afwijking van art §12.2 (14 px) op last van de eigenaar; de rekenbalk houdt zijn eigen maten. (4) Het prikbord draagt zijn knop bij naam (`klas` `hotbord`) en in zijn midden: met de grotere letters haalde de 📋-knop de GROOT-rekensom niet meer (13520 tegen 12960 was al nipt), schoof boven het bord en duwde het eerste taakkaartje 191 eenheden opzij. **Testverwachting bijgesteld:** `test_sleutels` MODUS 1024×768 `gewoon` → `stapel`, de hogere vraagkaart past niet meer boven de rij (op beeld nagekeken); ijkpunt in `test_de_voet_staat_waar_de_webbouw_hem_tekent` opnieuw uit de pixels gemeten: bij maat 18 staat de voet 22,0 onder de rijtop. **Nog open, eigenaarsbeslissing:** Hinkelen, Kraam en Was sorteren hangen waar hun spel zich afspeelt, maar hun stenen, kraam en wasstapel bestaan pas als het spel loopt — daarom zweven ze nog; en de taakkaartjes van het prikbord liggen met opzet schuin door de kamer (`Hotel.toon_bord`). **Bijvangst:** `test_hinkel.test_gasten_naast_de_strook_en_terug` is wisselvallig, ook op `512532f` (3 van 11 rood): een race tussen de eigen wandeling van de gast en de veegronde van het spel. Volle `tools/test.sh` 504 goed, 0 fout, nul SCRIPT ERROR.

- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Elke overgang laat zien waar hij heen gaat.** Bestanden: `autoload/rooms.gd`, `autoload/world.gd`, `scenes/vloer.gd`, `art/vloer.gd`, `art/decor.gd`, nieuw `art/decor_buiten.gd`, `tests/test_rooms.gd`, `games/hinkel/test_hinkel.gd`, specs `world.md` §1.2–1.4 en `art-sound-rules.md` §8, `HOTEL.md` §1. **De melding:** "Het zwembad vanuit de tuin gezien is niet duidelijk dat lijkt gewoon op een huis" en "Ook andere ruimtes als keuken naar tuin hebben geen mooie overgang die sprekend is … Grote veranderingen zijn hier toegestaan er is nog nooit goed over nagedacht". **Eerst nagelopen** (alle negen kamers op 1170×629): binnen was elke deur hetzelfde donkerbruine gat; de keuken had géén tuindeur (een `poort` in een kamer mét wanden snijdt geen gat, dus alleen een knop naast de koelkast); in de tuin stond de keukenpoort achter de boom en de zwembadopening achter het hok, zodat "Zwembad" op een huisje hing; vanuit het zwembad was de poort een stuk hek. Gemeten: de gangplanten verborgen 37 % van de deur van kamer 1 en 29 % van kamer 2, de plant in kamer 1 7 % van haar deur. **Nu:** (1) **Doorkijk:** door elke deur zie je de vloer van de kamer erachter (`scenes/vloer.gd` `doorkijk_tegels`, geknipt op wat de opening doorlaat), met de negge van de dikke wand, de drempel en een lateischaduw; binnen iets donkerder, van buiten naar binnen flink donkerder, naar buiten lichter en met een wit kozijn. Wie dezelfde vloer deelt richt zijn `kijk` op iets eigens: de receptie op haar roze kleed, de keuken op een nieuwe abrikozen loper voor het aanrecht, de wasserij op een nieuwe aquablauwe badmat. (2) **De tuin ligt áchter het hotel:** links de achtergevel (`Kamer.gevel`, geen zijhek meer, `hek_x` 0) met de keukendeur (open groen deurblad plat tegen de gevel, luifel, deurmat met pootafdruk, keukenraam met bloembak, stoep langs de muur, dak tot buiten beeld); achter het hek het zwembad zelf (`Kamer.uitzicht`: dek, rand, water, trapje, parasol) achter een witte zwembadpoort met reddingsboei en golfjesbordje. Het hok ging naar de hoek rechtsachter (naast de keukendeur las het als een tweede deur), de boom naar linksvoor, de kist tegen de gevel, een plant sluit de hoek tussen gevel en hek. (3) **Vanuit het zwembad** is de weg terug een rozenboog met de tuinboom en bloemstruiken achter het hek. (4) **De keuken** heeft een echte tuindeur, in de hoek na de koelkast (deur 104..116, de koelkast schoof naar x 94: vóór de oude opening verborg hij een derde van de deur). (5) De gangplanten staan langs de vóórrand (x 36 en 108, precies tussen de loopcellen: de gang houdt vier dwaalplekken), de plant van kamer 1 op (107, 8). (6) `Rooms.deur_hoog()` is de ene hoogteregel voor tekenen en meten; het vak van een poort ligt in de hekkenlijn, niet op de deurlijn erachter. **Nieuwe tests** (`test_rooms`): elke deur laat vloer van de bestemming zien, alleen wat de opening doorlaat, altijd een kleur die de eigen kamer niet heeft en nooit hetzelfde als een andere deur van die kamer; geen vast meubel verbergt meer dan 3 % van een deur (in pixels op g = 2); de tuin ligt achter het hotel en naast het zwembad; het vak van een poort ligt in de hekkenlijn. **Testverwachtingen bijgesteld:** deurpunt keuken → tuin (96, 0) → (110, 0); `hekz` 8 → 0 en `hekx` 7 → 8; de gouden pollentabel één keer opnieuw gezaaid uit een headless run (de pollen mijden nu elk vast decorstuk op het gras in plaats van een vaste lijst van vijf plekken); `test_hinkel` meet de verschuiving vanaf waar het hok nu staat. **Gevolgen voor open taken:** `R3` (de Kas) wilde een poort in de linker hekwand en een `hekz`-paal minder — die wand is nu de achtergevel, dus de Kas-poort moet een andere plek krijgen en stap 4 vervalt; `R4` vindt `parasol` en `zwembadtrap` al in `art/decor_buiten.gd` en hergebruikt die. **Nog open:** "Hinkelen" zweeft nog (zijn stenen bestaan pas als het spel loopt) en valt op de liggende telefoon en op 1170×629 over de pot van de hoekplant. Volle `tools/test.sh` 508 goed, 0 fout, nul SCRIPT ERROR; webexport schoon, alle negen kamers op 1170×629 en de buitenkamers en keuken op telefoon staand en liggend en op de iPad op beeld nagekeken.

- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Het dier springt in bed, de spellen laten hun spullen staan, de taakkaartjes hangen bij het prikbord.** Bestanden: `autoload/world.gd`, `autoload/games.gd`, `autoload/hits.gd`, `autoload/hotel.gd`, `autoload/rooms.gd`, `games/hinkel/spel.gd`, `games/kraam/spel.gd`, `games/was/spel.gd`, `tests/test_world.gd`, `tests/test_rooms.gd`, `games/was/test_was.gd`, `games/kraam/test_kraam.gd`, spec `world.md` §1.3, §2.7 en §5.1. **De wens:** "kan je ook een animatie maken dat het dier op het bed springt wanneer je een vrij bed kiest", en ja op "de stenen, de kraam en de wasstapel altijd laten zien" en op "de taakkaartjes bij het prikbord". **Nu:** (1) Een gast die naar zijn bed loopt, springt er vanaf de vloer naast het bed op: de boog van het hinkelpad (`_spring`), trager (tempo 0,7) en 4 voxels hoger, landend op het matras met de hurk en een `Snd.hup()`; pas dan ligt hij (`_spring_in_bed`). Zo ook na de check-in: kies een vrij bed, de gast loopt van de balie door de gang naar binnen en springt erin. Met minder beweging, en als hij al in de kamer is (laden, tests), ligt hij meteen. (2) `Games`: een spel kan in `definitie()` `modellen` (statische bouwers, bij de scan aangemeld) en `rust` (los decor) meegeven; het register zet die rustspullen neer zolang het spel speelbaar is en er géén spel loopt, onder eigenaar `rust:<id>` zodat het eigen decor van het spel en de tests die dat tellen ongemoeid blijven, en de ingang hangt eraan (`hotspot.rust`). Hinkelen: de elf kale stenen van band 3; Kraam: kraam en toonbank; Was sorteren: de wasstapel op de badmat. Bij de start van elk spel gaan alle rustspullen weg. De hinkelbouwers `_steen_model`/`_trap_model` zijn statisch geworden. (3) Het prikbord hangt aan de linkerwand boven het bankje in de wachthoek (`rot` 1): achter de balie had de zichtbare wand geen plek voor drie kaartjes, ze vielen steeds op de vloer. De kaartjes mikken nu alle drie op het bord en hangen eromheen (één erboven, twee eronder) in plaats van schuin door de kamer. (4) `World.vlak_van` draait mee met `rot`: Hits gaf een gedraaid stuk het vak van het ongedraaide. (5) Het hok staat weer in de achterhoek tegen de gevel (22, 22): rechtsachter zou de kraam, die nu blijft staan, het verbergen; de hoekplant is weg, het hok sluit die hoek. **Testverwachtingen bijgesteld:** de gouden pollentabel (15 pollen) opnieuw uit een headless run; `test_world`, `test_was` en `test_kraam` tellen na afloop alleen het eigen decor, niet de rustspullen. Nieuwe test `test_de_gast_springt_op_het_bed`. **Nog open:** "Was sorteren" hangt onder de stapel en raakt de bovenkant van de tobbe, die in dezelfde schermkolom staat; twee taakkaartjes hangen onder het bord boven de vloer van de wachthoek, niet op de wand; op de staande telefoon (360×740) staat de 📋-knop schuin onder het bord in plaats van erop en "Sleutels" iets verder van het sleutelbord — de linkerwand is daar voller geworden. Volle `tools/test.sh` 509 goed, 0 fout, nul SCRIPT ERROR; webexport schoon; tuin, wasserij en receptie op 1170×629 en de telefoon nagekeken, de sprong in bed als filmpje vastgelegd via de echte route (bel, kies een vrij bed).

- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Elk woord hangt aan het ding waar het over gaat.** Bestanden: `autoload/hits.gd`, `autoload/hotel.gd`, `autoload/ui.gd`, `autoload/world.gd`, `autoload/rooms.gd`, nieuw `ui/prikbordblad.gd`, `ui/blad.gd`, `ui/kaart.gd`, `ui/teksten.gd`, `art/decor_slaapkamer.gd`, `games/bedden/spel.gd`, `games/wekker/spel.gd`, `games/sleutels/spel.gd`, `games/kraam/spel.gd`, `games/meubels/spel.gd`, `tests/test_hits.gd`, `tests/test_games.gd`, specs `world.md` §1.6, §3.3, §3.7, §5.1, §5.4, §6.3, §7.7, `games-a.md` §4.4, `games-b.md` §2.2, §2.7, `art-sound-rules.md` §8, `HOTEL.md` §9. **De melding:** "Plaatjes als [de receptie met het open prikbord] zijn veel te druk in iconen. en wekker zetten is bijvoorbeeld helemaal niet relevant aan waar de tekst geplaatst is. Dit gebeurt vaak over het hele spel en is een probleem kan je dat allemaal nakijken en fixen". **Eerst nagelopen** (`tmp/audit_labels.gd`: elke kamer, inchecken, avond, afrekenen en alle tien spellen op 1536×760, met per knop de afstand tot zijn eigen ding en wat hij van andere dingen afdekt): de taakkaartjes hingen 53 en 157 px van het bord midden in de receptie; tien knoppen stonden op een ánder ding ("Keuken" op de kist in de gang, "Receptie" op de ballenbak, "Sleutels" op de plant, "Keuken" op de strijkplank, "Gang" op het bankje, "Was sorteren" op de tobbe, "Kraam" op een hinkelsteen); de incheckgast wachtte bij het bankje in de verre hoek terwijl zijn kaart aan de balie hing; de families van de avondronde zweefden midden op de vloer; "⏰ Wekker" hing op het pootjesschilderij en "🛏 Bedden" op de speelmand, naast "Speelmand"; de wekkerkaart hing 290 px onder de klok; de munten van de kraam lagen een kwart scherm links van de kraam; de buidel van het meubelboek lag linksvoor in de hoek; "Leeg" stond naast in plaats van onder het bakje; de sleutelrij zakte op het kleed zolang de gast achterin stond. **Nu:** (1) Het **prikbord is een blad** (`Ui.prikbord_blad`, `ui/prikbordblad.gd`): de kaartjes als briefjes op kurk, elk met "in de keuken" / "in kamer 1" / "bij het zwembad" eronder, de dagberichten eronder, `Sluiten`; op een laag scherm naast elkaar; niets van het bord hangt nog in de kamer; het opent waar je bent, wacht achter het startscherm tot dat dicht is en bouwt zich zonder pop-in opnieuw als een kaartje verandert. Het bord hangt weer aan de achterwand (12, 1). (2) `Hits` `op: "aan"`: onder, boven, ernaast, gecentreerd op het ding zoals het getekend is; de eerste plek die van geen ánder ding (vast decor, slots, dingen, los decor, gasten) 15 % of meer afdekt wint, anders de minst erge; een `aan`-knop kiest elke beurt opnieuw. (3) Een **deurbordje hangt óp zijn deur** (midden, anders aan de voet in de opening), op zijn eigen deur telt niets ernaast. (4) **Aan de balie:** de gast checkt in op `BALIEPLEK` vóór de balie, de vraagkaart houdt daar het vak van de gast vrij, het afrekenen loopt naar dezelfde plek, de families wachten vóór de balie, het "🛏 Kamer 1"-wolkje hangt aan de deur waar de weg begint; de balie die kaarten mijden is zijn stukken (`World.vlakken_van_balie`) in plaats van het vak om de schuine balie, dat vooral vloer was. (5) **Rustspullen:** de wekker laat zijn klok (op 7 uur) in de gang hangen, het bedden-spel een nieuwe **dekenkist** tussen de bedden (`dekenkist`, lila met een deken en een kussen); de ingangen hangen eraan. (6) Een **vaste kaart** die op haar eigen ding zou liggen stapt ernaast in plaats van banden weg; de wekker meet zijn kaart eerlijk (`Ui.kaart_mat`, de eigen minimummaat was een toren) en laat een band vrij voor "10 uur"; een kaart is zo breed als haar hulpregel, tot haar maximum, zodat die op één regel blijft (K3). (7) De kraam telt haar munten vanaf de kolom van de toonbank; het meubelboek legt buidel, "klaar" en "terug" rond zijn toonbank; `Ui.wolk` en `Ui.bron` geven `op` en `obj` door, zodat een wolkje of bron `aan` zijn ding kan hangen. (8) Sleutels: opstelling `rechts` — duwt de gewone opstelling de rij van het bord af, dan hangt de rij vlak boven het bord met de kaart ernaast. (9) Een blad dat sluit geeft zijn naam `Blad` meteen op. **Nieuwe tests:** `test_games` elk spel hangt aan zijn eigen rustspul (weg zodra een spel start, terug na afloop); `test_hits` elk bordje op zijn deur en elke knop aan zijn ding in alle negen kamers op vier schermen (op de telefoon mag het één band), het prikbord is een blad (niets van het bord in de kamer, elk briefje noemt zijn kamer, een tik gaat erheen), de gast checkt in aan de balie. **Nog open:** staand hangt de sleutelrij vlak boven de rekenbalk, niet bij het bord; in de gang op de telefoon hangen twee deurbordjes net boven hun deur en de bedknoppen van kamer 1 tot een band van hun bed; tijdens het inchecken staat "Bel" rechts van de bel bij de kassa; start je een spel terwijl er een gast aan de balie staat, dan hangen beide kaarten in beeld (bestond al). Volle `tools/test.sh` 513 goed, 0 fout, nul SCRIPT ERROR; webexport schoon; alle kamers en de momenten op 1536×760, de telefoon staand en liggend en het blad in de browser (1536×760 @2,5) nagekeken.

- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — De sleutelhaakjes hangen langs de muur.** Bestanden: `games/sleutels/spel.gd`, `games/sleutels/test_sleutels.gd`, `autoload/ui.gd`, specs `games-a.md` §4.4 (nieuwe opstelling E `wand`) en `world.md` §5.5 (`balk: false`). **De wens:** "ja" op "zal ik het sleutelspel ombouwen zodat de haakjes langs de muur hangen?" — tot nu toe hingen de plaatjes in een waterpasse rij op de diepte van het bord, en zo'n rij loopt de kamer in in plaats van langs de muur: aan de linkerwand van de receptie paste ze niet tussen bord en gangdeur en zakte ze naar de achterwand of het kleed. **Nu:** opstelling `wand` is de eerste keus: de plaatjes hangen óp de muur van het sleutelbord, op één hoogte, van boven de voorkant van het bord naar achteren, boven de koffer en het bankje; op het scherm loopt de rij schuin omhoog en lees je de getallen van links naar rechts op volgorde. De hoogte is de laagste waarop de hele rij vrij hangt; het bord en de deuren van die muur tellen met hun echte schuine vorm (hun schermrechthoek is voor de helft lege muur). De kaart krijgt een vrije plek bij de rij (onder het begin, vóór bord en bankje). `Ui.somkaart` kent `balk: false`: op een ruim liggend kader gaat de sleutelkaart nooit de rekenbalk in, want de ophangkaart zou er wél in gaan en de vraagkaart niet, de wereld zou tussen de twee stappen krimpen en de haakjes zouden van de muur naar een rij bovenin springen. Staand gaan beide kaarten de balk in en hangt de rij hoog boven de gangdeur langs; op een telefoon passen vier tikdoelen niet langs één muur en blijven de opstellingen A–D. **Testverwachtingen bijgesteld:** `MODUS` 1024×768 en 768×1024 `stapel` → `wand`; `_keur_rij` kent de muurrij (elk plaatje rechts van én hoger dan zijn buurman, gelijke stappen, geen overlap). **Nieuwe test** `test_de_haakjes_blijven_aan_de_muur` (1024×768: `wand` in de reken- én de ophangstap, de plaatjes op dezelfde plek, de ophangkaart niet in de balk). Op beeld nagekeken: 1536×760, 1024×768 en 768×1024, reken- en ophangstap, groep 3 en groep 5.
- 2026-09-23 (Claude Opus, tak `nieuwe-kamer`, niet gemerged) — **EIGENAARSWENS 2026-09-23 — Een nieuwe kamer met eigen rekenspellen: de Kas 🪴 (`R3`), met Aardbeien plukken en Groenten wegen.** Bestanden: `autoload/rooms.gd`, `scenes/vloer.gd`, `art/decor.gd`, nieuw `art/decor_kas.gd`, `ui/plattegrond.gd`, `ui/kamerbalk.gd`, `autoload/snd.gd`, `fonts/tekens.txt` en `fonts/Emoji.ttf`, nieuw `games/oogst/` en `games/weeg/` (elk `spel.gd`, `spel.tscn`, `beurt.gd`, `modellen.gd`, `test_*.gd`, met `.uid`), `tests/test_rooms.gd`, `tests/test_plattegrond.gd`, `tests/test_sfeer.gd`, `tests/test_kamerbalk.gd`, nieuwe spec `games-c.md`, `world.md` §1.2–1.5 en §5.1, `art-sound-rules.md` §8–9, `HOTEL.md` §1 en §5, `tools/kiek.js`. **De wens:** "Kan je met een aparte agent nog een nieuwe map verzinnen en extra reken minigames daarvoor?" **Nu:** (1) **De Kas** (128 × 112, `tegel`, `loop` 1,25) is een glazen kas achter het hotel. De tuin krijgt een glazen deur in de achtergevel, waar het keukenraam hing, met een glazen luifel erboven en een potplant ernaast (het linkerhek uit `R3` is sinds 2026-09-23 de gevel). Binnen: glazen wanden op een bakstenen borstwering (`Kamer.glas`, `scenes/vloer.gd`; een deur naar de kas krijgt een wit kozijn), een terracotta pad van de tuindeur naar voren (daar kijk je door de deur op), twee moesbakken langs de achterwand (zone `rijen`), een zonnebloem, twee hangplanten, een potkast, een zaadkist, een gieter, een kruiwagen, pompoenen en de aardbeienbak langs de linkerwand. Plattegrond `[4, 1]` boven de tuin, sfeer `warm`, een chip in de kamerbalk. Nieuw `Kamer.mijd`: de vloer onder de tafels van de twee spellen krijgt geen loopplek en geen gekocht meubel. (2) **Aardbeien plukken** (`oogst`; ingang "🍓 Plukken" aan de aardbeienbak, de pluktafel met bakjes van tien blijft staan als rustdecor): "Hoeveel aardbeien liggen er?" (tientallen en eenheden aflezen), "Hoeveel passen er nog bij?" met `7 + __ = 10` (groep 5: "Hoeveel nog tot 100?" met `63 + __ = 100`), dan zelf plukken met "🍓 Pluk"; groep 5 plukt na het volle bakje een heel bakje per tik (via de tien). Groep 3: één open bakje, of één vol en één open (tot 20, splitsen tot 10); groep 4: 2–5 volle bakjes (tot 100, aanvullen tot het tiental); groep 5: 5–8 volle bakjes (aanvullen tot 100); de volle bakjes groeien mee met het aantal gasten. Afleiders: de cijfers omgedraaid (41 voor 14), de cijfers opgeteld, een bakje te veel. Hulp: "💛 10 … 20 … 30 en nog 7" (een wolkje aan de tafel, op de telefoon een regel op de kaart), daarna een bleek spookgetal. (3) **Groenten wegen** (`weeg`; ingang "⚖️ Wegen" aan de marktbalans die blijft staan): lees af wat de gewichten samen wegen ("Hoeveel kilo is de pompoen?"), leg zelf gewichten op tot de balans recht staat (hij helt naar de zwaarste kant: "⬆ nog te licht", "⬇ te zwaar", "⬆ eraf" aan de balans) en lees je eigen meting af. Gewichten 1, 2, 5 kg (tot 10) · 1, 2, 5, 10 kg (tot 20) · 1, 5, 10, 20 kg (tot 60, de reuzenpompoen); het tweede ding wordt zwaarder met het aantal gasten. De gewichten liggen als voorraad op de vloer en staan op de antwoordstrook ("⬇ 5"): losse wereldknoppen per gewicht kwamen te ver van hun gewicht te hangen. (4) `ui/kamerbalk.gd` (buiten `R3`): elf chips passen op 1024×768 niet op één rij met de gewone vulling, en een tweede rij kostte het kader 52 eenheden hoogte (990×637 → 990×585) en daarmee de incheckgast in `test_hits`. Een rij probeert nu eerst `VULLING_KRAP` (4) en breekt pas als ook die niet past; een rij die past houdt haar vulling. (5) 🍉 🍓 🎃 🥔 in de fontsubset (`Emoji.ttf` opnieuw gebouwd; met de oude lijst gaf dat een byte-gelijk bestand). (6) Twee vragen zijn na het nakijken op de telefoon korter gemaakt: "Hoeveel kilo is de pompoen?" (niet "… weegt de pompoen?") en "Hoeveel nog tot 100?" (niet "… tot 100 aardbeien?"). Anders was de regel op een scherm van 360 breed te lang voor de rekenbalk en zweefde juist die kaart boven de kamer, terwijl de rest van de beurt in de balk stond. **Testverwachtingen bijgesteld:** alleen rijen erbij (`ORDE`, maten, kader en deurpunten van de kas) en `test_plattegrond` deurparen 8 → 9 voor de nieuwe deur; geen gouden tabel opnieuw gezaaid. **Nieuwe tests:** `test_oogst` (15) en `test_weeg` (13), met elk een hele beurt per band via de echte knoppen, de hulpladder, herladen, geen gasten, het rustdecor en de knoppen op vier schermen (elke knop bij zijn ding; de vragen in de rekenbalk op 1024×768, 768×1024 en 360×740, en `test_groep_5_op_de_telefoon`); `test_de_kas_is_een_glazen_kas_achter_het_hotel`; `test_elf_chips_op_een_rij_op_de_tablet`. **Afwijkingen van `R3`:** 128 × 112 in plaats van 110 × 96, en 13 dwaalplekken in plaats van ≥ 16 (twee spellen en elf decorstukken; de kamertest eist ≥ 8); de deur zit in de gevel, niet in een linkerhek (stap 3 en 4 vervielen door de tuinverbouw); het keukenraam in de tuin is weg; stap 5 (het hinkelpad verschuiven, `test_het_pad_raakt_de_poort_niet`) is **niet gedaan**. **Nog open:** op de liggende telefoon (740×360) vindt het S5-wolkje "🔄 Nog een keer" van `Ui` geen ruime plek (beide spellen zonderen alleen dat daar uit in hun schermtest), en op een telefoon gaat de kaart uit de rekenbalk zolang er een hulpregel op staat (de kaart wordt dan hoger dan een derde van het kader). `test_hinkel` `test_gasten_naast_de_strook_en_terug` is wisselvallig, ook op `main`: `_speel_verder` start en stopt het spel (samen ±70 ms rekenwerk) en wacht dan twee frames, en vallen daarin twee wereldtikken (1/15 s) dan is de pose van hk2 al verlopen en wandelt hij zelf verder ("hk2 krijgt zijn eigen gang terug: kreeg loopA, verwacht rust") — het hangt af van de fase van de tikklok. Met `godot --frame-delay 40` faalt hij op `main` (a64da0a) precies zo, en start en stop duren op `main` en op deze tak even lang (44–57 ms en 17–21 ms). Op deze tak waren daardoor twee van de vijf volle runs rood (elke keer alleen deze test); de laatste, op de eindstand, is groen. Niet aangeraakt: niet van deze taak; de test zou `World.pauzeer(true)` kunnen zetten tijdens die twee frames. Volle `tools/test.sh` **544 goed, 0 fout** (op `main` 514; +30 nieuwe tests), nul SCRIPT ERROR, 3 verwachte motorfouten zoals op `main`; `core/sommen.gd` byte-gelijk; beide spellen op 1536×760 en 360×740 op beeld nagekeken in groep 3, 4 en 5. `R3` en de batchtabel blijven open tot de tak op `main` staat.

- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Geen knop die bij een tik niets kan.** Bestanden: `autoload/hotel.gd`, `autoload/games.gd`, `games/bedden/spel.gd`, `games/voerkar/spel.gd`, `games/meubels/spel.gd`, `tests/test_hits.gd`, `tests/test_games.gd`, `games/bedden/test_bedden.gd`, `games/voerkar/test_voerkar.gd`, specs `world.md` §5.1 en §6.3, `games-a.md` §4. **De melding:** "actions such as a blank bed are available after entering a guest but when you click on them you can't execute them ... this provides visual clutter". **Eerst nagelopen** (een Opus-verkenner las alle tien spellen): een vrij bed gaf buiten het inchecken alleen "🛏 Bel eerst een gast", de speelmand zonder speler "🧶 Straks samen spelen", een leeg bakje "🍪 Vul eerst de voerkar" of "🍽 Hier slaapt niemand", de bel met volle bedden of een gast aan de balie "alle bedden vol" / "Er staat al iemand"; van de spellen komen alleen bedden ("vol ✓", of de beurt van vandaag nog eens), voerkar ("Alle bakjes vol!") en meubels ("Nog geen munten") via hun knop in zo'n toestand — de "nog geen gasten"-paden van de andere zijn onbereikbaar (`unlock` vraagt gasten en elke gast heeft een bed); niemand sleept iets op een bed of bakje. **Nu:** buiten een spel staat een hotelknop er alleen als een tik iets doet: de bel met een vrij bed en niemand aan de balie, een vrij bed alleen bij het kiezen van een bed (stap 3 tekent de knoppen opnieuw), de speelmand als hier iemand wil spelen, een bakje met eten en een gast die nog moet eten; tijdens een spel alles zoals het was (de voerkar leent bakjes en deuren). `definitie().kan` (`Games.speelbaar_nu`) verbergt de ingang en het taakkaartje van een spel zonder iets te doen: bedden (geen plek voor twee bedden, of de beurt van vandaag is af), voerkar (geen leeg bakje waar iemand slaapt), meubels (geen munt, geen ster, niets neer te zetten). **Fout gevonden en hersteld:** de voerkar las 's ochtends de kar van gisteren uit zijn laatje (`Hotel.morgen()` wiste alleen `state.kar`) en zei "Alle bakjes vol!" bij lege bakjes; een kar draagt nu zijn dag. **Testverwachting bijgesteld:** `test_bedden` controleert na afloop het deurbordje in plaats van de speelmand (die heeft alleen nog een knop als iemand wil spelen). **Nieuwe tests:** `test_hits` een knop staat er alleen als hij iets doet; `test_games` een spel zonder iets te doen heeft geen knop en geen kaartje; `test_voerkar` de kar van gisteren telt niet meer.
- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — `👀 Volg`: de camera loopt met een aankomende gast mee.** Bestanden: `autoload/hotel.gd`, `autoload/ui.gd`, `ui/wolk.gd`, `tests/test_komt.gd`, `fonts/tekens.txt` en `fonts/Emoji.ttf` (👀 U+1F440 in de subset — `test_tekens` ving het ontbrekende teken; alleen de emoji-subset herbouwd, met dezelfde fontTools-opdracht als `fonts/maak-fonts.sh`, die zonder 👀 byte-gelijk de oude oplevert), spec `world.md` §2.10 en §6.3, `README.md`, `AGENTS.md`. **De wens:** "wanneer een dier eraan komt de "volg" knop die met de camera het dier volgt tot het gearriveerd is in de map van bestemming". **Nu:** de "komt eraan"-bubbel draagt rechts een geel knopje `👀 Volg` en de hele bubbel is de knop (`Ui.wolk` kent `knop`; er komt geen los knopje bij dat een plek in het raster zoekt). Een tik zet de camera in de kamer waar het dier nu is; bij elk getekend beeld gaat de camera mee door de deur waar het dier doorheen stapt (`Hotel.naar_kamer`, dus met deurgeluid en een nieuwe tekening), tot het in de kamer van bestemming staat — daar stopt het volgen vanzelf, en dat is de kamer waar het kind keek. Zolang je volgt staat er `👀` voor zijn naam op het naambordje. Zelf een kamer kiezen (deur, kamerbalk, plattegrond) of een spel dat begint stopt het volgen; tijdens een spel volg je niemand. **Nieuwe tests** in `test_komt`: volgen loopt mee tot hij er is (kamer1 → gang → receptie, de camera steeds in zijn kamer, daarna stil), zelf een kamer kiezen stopt het volgen, een spel stopt het volgen. Op beeld nagekeken: 1536×760, 412×915 en 915×412 met de langste naam. **Bestaand, niet veranderd:** op een liggende telefoon (915×412) staat de bubbel onderin de kamer in plaats van bij de gangdeur — dat deed hij vóór het knopje al.
- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Wisselen met welk dier je speelt.** Bestanden: `spel/minigame.gd`, `spel/ctx.gd`, `autoload/games.gd`, `ui/spelbalk.gd`, `ui/teksten.gd`, `scenes/main.gd`, `games/sleutels`, `kraam`, `hinkel`, `zwembad`, `wekker`, `tobbe` (`spel.gd` en `test_*.gd`), `tests/test_games.gd`, specs `world.md` §4.2, §5.2, §5.3 en nieuw §5.8, `games-a.md` §4.1 en §6.5, `games-b.md` §0.12, §1.1, §2.1, §3.1, §5.1 en §5.12. **De wens:** "een methode om te wisselen met welk dier je de spellen speelt". **Nu:** loopt een spel met één dier van de beurt, dan staat dat dier op de spelbalk na de spelnaam: `🐶 Boef 🔄` (in de rail `🐶 🔄` boven `Boef`), even groot als `⬅ Terug`, en alleen als een ander dier de beurt kan overnemen. Een tik geeft de beurt aan het volgende dier in check-in volgorde en start het spel opnieuw — dezelfde stop-en-start als verdringen: een verse beurt voor dat dier, er gaat geen ster of munt af, de beurt van het vorige dier is gewoon niet af. De keuze staat alleen deze sessie in `State.s.speler` (`State.lees()` neemt hem niet over) en gaat bij elke volgende start vóór de eigen keuze van het spel, zolang dat dier mee mag doen — ook in de andere spellen. Een bewaarde beurt van een ander dier wordt dan niet hervat; wie zijn beurt niet afmaakte maakt plaats (`ctx.laat_gaan`: van de toonbank of het dek af, of terug naar bed als hij nog onderweg was; een slaper wordt nooit gewekt). Contract: `MiniGame.spelers()`, `ctx.speelt/voorkeur/laat_gaan/wissel_speler`, `Games.speler/spelers/volgende_speler/kan_wisselen/wissel_speler`, signaal `Games.speler_veranderd`. Sleutels legt hetzelfde bord opnieuw met de eerste sleutel voor het gekozen dier (de kern zaait op dag, N, band en ronde, niet op het dier); kraam laat ook een gast zonder 🎁-wens kopen (zonder wens); wekker begint een verse ronde van hooguit drie bij het gekozen dier. **Afwijkingen van de schets:** (1) ⇄ (U+21C4) zit in geen van de drie fontsubsets en ook niet in Noto Sans Symbols 2 — tofu — dus 🔄, het huisteken voor "nog een keer"; (2) tobbe kreeg géén dierknop: het heeft geen dier van de beurt (de som gaat over sop en tobbes). Wel gevonden en opgelost: in de badstap zette een tik op een wachtend dier altijd de EERSTE wachtende in bad (het slepen uit de HTML is nooit meegekomen); nu gaat het aangetikte dier erin; (3) de getallen van de verse beurt zijn die van de dag, dus vaak dezelfde som met een ander dier. **Nieuwe tests:** `test_games` `test_het_dier_van_de_beurt_gaat_rond`, `test_de_spelbalk_wisselt_van_dier` (1536×760, 1024×768, 360×740, 740×360: vol tikdoel in de balk, kader beweegt niet, tik → volgend dier), `test_wie_zijn_beurt_niet_afmaakt_maakt_plaats`; per spel `test_het_kind_kiest_wie_…` en tobbe `test_wie_je_aantikt_gaat_in_bad`. Geen bestaande verwachting veranderd. Volle `tools/test.sh` 523 goed, 0 fout. Op beeld nagekeken (xvfb, 1536×760 en 360×740): sleutels, zwembad, hinkel en kraam vóór en na een tik; wekker (één slaper) en tobbe zonder knop. **Gezien, niet van deze taak:** `test_hinkel.gd::test_gasten_naast_de_strook_en_terug` is tijdgevoelig (1× rood in 5 losse runs: een gast die na `stop()` al weer dwaalt staat niet op `rust`; die test zet geen keuze); op 360×740 staat de sleutelkaart bleek bovenin, ook op `main`.

- 2026-09-23 (Claude Opus) — **Samenvoegen van de drie eigenaarswensen van 2026-09-23** (knoppen + `👀 Volg`, het dier van de beurt, de kas). Bestanden: `games/oogst/spel.gd`, `games/weeg/spel.gd` en hun `test_*.gd`, `games/hinkel/test_hinkel.gd`, `fonts/Emoji.ttf`, specs `world.md` §5.8 en `games-c.md` §2.9 en §3.9. **De kas-spellen kennen het dier van de beurt:** plukken en wegen hebben elk één gast van de beurt, dus ook daar staat `🐶 Boef 🔄` op de spelbalk — `spelers()` = elke gast met een bed in check-in volgorde, `ctx.voorkeur` bij de start, `ctx.speelt`, en wie zijn beurt niet afmaakte maakt plaats (`ctx.laat_gaan`); nieuwe tests `test_het_kind_kiest_wie_er_plukt` en `test_het_kind_kiest_wie_er_weegt`. **Emoji-subset** opnieuw gebouwd uit de samengevoegde `tekens.txt` (👀 én 🍉 🍓 🎃 🥔; beide takken bouwden hem byte-gelijk na met dezelfde fontTools-opdracht). **Wisselvallige test rechtgezet:** `test_hinkel` `test_gasten_naast_de_strook_en_terug` wachtte twee beelden tussen stop en start terwijl de wereld doortikte; op een trage machine ging een losgelaten gast al dwalen (`loopA` in plaats van `rust`) — met `godot --frame-delay 40` elke keer rood. De wereld staat die twee beelden nu stil (`World.pauzeer`): traag 3 × groen, gewoon groen.
- 2026-09-23 (Claude Opus) — **EIGENAARSWENS 2026-09-23 — Gasten komen binnen door de voordeur, elk dier op zijn eigen manier.** Bestanden: `autoload/rooms.gd` (alleen het receptieblok, plus `Kamer.ingang` en `Rooms.ingang`), `autoload/world.gd` (`kom_binnen`, `komt_binnen`, `ingang_open`, `vlak_van_ingang` en de speler van de binnenkomst, toestand `komt`), nieuw `wereld/binnenkomst.gd` (de choreografie per soort, als lijst van tellen), `art/gasten.gd` (zes eigen houdingen in `POSE_EXTRA` en `bukken`; de vijftien gouden houdingen onaangeroerd), `art/decor_hotel.gd` (`voordeur`, `welkomsmat`), `scenes/kamer.gd` (de deur open en dicht), `autoload/hotel.gd` (alleen de regels van `bel()` die de nieuwe gast neerzetten en sturen: 2 weg, 1 anders), `tests/test_binnenkomst.gd` (nieuw), `tests/test_rooms.gd`, specs `world.md` §1.1–§1.3, §1.5, §2.4–§2.5 en §3.3, `art-sound-rules.md` §5.2, §8 en §11.8. **De wens:** "En nog een agent om het binnenkomen van dieren te verbeteren die komen momenteel vanuit de gang binnen ipv ingang. Doe ook een leuke animatie wanneer ze binnenkomen dat per dier anders is". **Nu:** de receptie heeft een voordeur rechts in de achterwand, voorbij het eind van de balie — het enige vrije stuk wand in beeld (links zitten de gangdeur, het bankje, het sleutelbord en de speelzaaldeur, achter de balie het prikbord): wit kozijn, mintgroen blad met een groot raam vol lucht, een bovenlicht met een zonnetje en een roze welkomstmat met een hartje, en daarmee de enige dichte, glazen deur van het hotel. Ze is geen deur van de graaf: geen pad, geen deurknop, geen chip, geen vak op de plattegrond; buiten is geen kamer. Wie aanbelt verschijnt op de drempel; de deur staat open (lucht, heg en stoep door de opening) tot hij 22 voxels binnen is en valt dan dicht met `Snd.deur`, en hij gaat om het eind van de balie naar zijn plek: de puppy springt kwispelend naar binnen en draait aan de balie vier rondjes, de poes sluipt laag naar binnen, rekt zich aan de balie uit ("mrrp") en gaat zitten, het konijn huppelt het hele stuk en maakt aan de balie één grote draaisprong, de gans gakt en fladdert in de deuropening, waggelt fladderend naar binnen en fladdert nog eens aan de balie. Elk 3,9 s tot `BALIEPLEK(0)` en elk eindigt in de gewone wachthouding. De incheckkaart staat er meteen, zoals voorheen; wie al een bed kiest terwijl de gast nog binnenkomt, stuurt hem meteen naar bed. Rustmodus en buiten beeld: geen binnenkomst, hij staat meteen aan de balie. **De families bij het uitchecken** lopen niet — het zijn de `👪`-wolkjes aan de balie, en de gast die ze komen halen loopt uit zijn eigen kamer door de gang — dus daar is niets veranderd. **Keuzes:** de deur is een wandmodel (open/dicht via `params.open`) en geen gat in de muur, zodat `scenes/vloer.gd` ongemoeid blijft; de poster met de poot bleef boven de deur hangen, als uithangbord; geen nieuw geluid (`snd.gd` was niet van deze taak): `hup`, `terug` en `plop` doen het werk. **Gevonden, niet van deze taak:** in de avondronde en bij de rekening kan een knop die in een band wordt gezet (`rek_ok`, `rek_buidel`) of `🌙 Avond` op de balielamp de voordeur deels afdekken — gemeten tot 69 % op 740×360, 44 % op 768×1024 en 42 % op 1536×760 — omdat `Hits` alleen dingen mét een knop vrijhoudt; één regel in `Hits.plaats()` (`World.vlak_van_ingang()` bij `_vakken`) lost het op, `hits.gd` hoorde niet bij deze taak. **Nieuwe tests:** `test_binnenkomst` (elk dier komt anders binnen — vier verschillende houdingsreeksen, sprongen, draaien en tijd; de voordeur staat open tot hij binnen is; nooit door de balie; de bel haalt de gast door de voordeur en de check-in wacht niet; snel een bed kiezen; een ander bevel gaat voor; buiten beeld; rustmodus; de zes houdingen; de deur open en dicht; geen knop op de voordeur in rust en bij elke vraag van de check-in op zes schermen) en in `test_rooms` de voordeur buiten de deurgraaf, niets voor de voordeur, de weg van de voordeur om de balie. **Geen bestaande verwachting veranderd** (de helper `_deur_dekking` kreeg alleen een optioneel argument). Volle `tools/test.sh` 575 goed, 0 fout (main: 561). Op beeld nagekeken (xvfb, elke 0,15 s): alle vier binnenkomsten op 1536×760 en 360×740, en de receptie op 1024×768 en 740×360 in rust, bij het inchecken en in de avondronde.

- 2026-09-24 (Claude Opus) — **Samenvoegen: de voordeur is een ding voor de knoppen.** Bestanden: `autoload/hits.gd`, `tests/test_binnenkomst.gd`, `AGENTS.md`. De voordeur van de binnenkomst-tak draagt geen knop, dus de bandplaatsing hield er geen rekening mee: in de avondronde dekte `🌙 Avond` er tot 69 % van (740×360) en de rekening 42 % (1536×760, gemeten op die tak). `Hits.plaats()` neemt `World.vlak_van_ingang()` nu op in de vakken die een knop mijdt. `test_geen_knop_op_de_voordeur` kijkt ook naar de avondknop; op beeld nagekeken in de avondronde en bij de rekening op 740×360, 768×1024 en 1536×760 (de headless test haalt die dekking zelf niet: daar staat de lamp op elk scherm al vrij). `AGENTS.md` noemt `World.kom_binnen`, `World.vlak_van_ingang` en `Rooms.ingang`.

- 2026-09-23 (Claude Opus) — **EIGENAARSWENSEN 2026-09-23 (avond) — Vraag en antwoorden samen, wensen wijzen de weg, de ochtend is de bel, knop of mededeling.** Bestanden: `autoload/hits.gd`, `autoload/hotel.gd`, `autoload/ui.gd`, `autoload/games.gd`, `autoload/snd.gd`, `ui/kaart.gd`, `ui/keuzestrook.gd`, `ui/hotknop.gd`, `ui/wolk.gd`, `ui/thema.gd`, nieuwe test `tests/test_wens_tik.gd`, tests `test_hits`, `test_balk`, `test_ui`, `test_snd`, spec `world.md` §5.5, §5.6, §6.2, §7.5 en §7.6. **De meldingen:** "de text van opdrachten staat soms ver van waar ik kan klikken voor antwoorden" · "Wanneer boef nu eten wil weet ik niet waar ik naar toe moet. Als ik eten druk gebeurt er niks dat zou me naar de keuken moeten brengen" · "geef de bel een ding geluid en laat niet het prikbord zien als start. De gebruiker moet gewoon op de bel drukken" · "bij het voeren staat er soms heel veel textboxen die allemaal klikbaar lijken" · "De manier om voer scheppen te tellen is ook niet heel duidelijk". **Nagelopen:** op 1280×720 stond de inchecksom rechtsboven bij de balie en haar strook onderin: een drijvende kaart mag over de kamer, haar strook niet over een ding, en onder de kaart stond de balie; op 900×600 en 820×600 dokte de kaart in de lage balk en verloor daar haar tweede regel ("📦 In huis: 40 scheppen. Genoeg?"). **Nu:** (1) kaart en strook worden als één paar geplaatst: de kaart neemt de eerste plek in haar gewone volgorde waar haar strook er direct naast past, en die plek wordt voor de strook bewaard; waar de strook al paste verschuift niets; past er nergens een paar, dan gaan ze samen naar de voet. Het sleutelbord plant kaart, haakjesrij en strook zelf en zegt `paar: false` (anders brak op een telefoon de haakjesrij in tweeën: het spel legt zijn rij rond de plek waar het de kaart verwacht); daar blijft op een telefoon de strook soms los van de kaart staan — dat vraagt een aanpassing in de opbouw van het sleutelbord zelf. De lage balk neemt geen kaart met een tweede regel. In de balk eerst de lange woorden ("te weinig", "blijft over"). Een melding staat boven de balk, niet over de antwoorden. (2) Een tik op een wens brengt je waar hij uitkomt, zoals zijn kaartje op het bord: 🍪 met eten in zijn bakje → zijn kamer, het bakje knippert zacht; 🍪 met een leeg bakje → de keuken met de voerkar open; 🧶 → zijn kamer, de speelmand knippert; een wens van een spel → dat spel. (3) De receptiebel zegt `Snd.ding()` (aangeslagen belletje, ±1 s naklank; `Snd.bel()` houdt zijn HTML-referentie), het prikbord gaat nooit meer vanzelf open (niet bij de start, niet 's ochtends) en de bel ademt zolang het ochtend is. (4) Knoppen staan op een warme rand met een dikkere drukrand en schaduw die zakt als je drukt; een wolk met een eigen tik draagt dezelfde rand; een mededeling is plat warm papier, zonder rand en schaduw, reageert niet en laat een tik door. (5) Inchecken: "Elke dag eten de gasten 12 scheppen" / "Boef wil er 3 bij. Hoeveel samen?" en "Voer voor 4 dagen: elke dag 15 scheppen" / "📦 In de kast: 40 scheppen. Genoeg?". **Fout gevonden en hersteld:** `Games.start` tekende de hotelknoppen vóór de camera naar de spelkamer ging; een spel dat van elders begon vond daardoor geen deur van zijn eigen kamer om te lenen (de voerkar zonder deuren). **Nieuwe tests:** `test_hits` kaart en antwoorden staan samen (faalt zonder het paar op vier kaders), `test_balk` lage balk neemt geen tweede regel, `test_ui` knop en mededeling zien er anders uit, `test_snd` de receptiebel zegt ding, `test_wens_tik` (vier: leeg bakje opent de voerkar, eten in het bakje wijst het aan, spelen wijst de mand aan, de ochtend is de bel). Op beeld nagekeken: inchecken op 1536×760, 1366×768, 1280×720, 1024×640, 900×600, 820×600 en 740×360; kamer 1 en de voerkar op 1536×760.

- 2026-09-23 (Claude Opus 5.5, tak `wacht-op-dier`) — **EIGENAARSWENS 2026-09-23 — De minigame begint pas als het dier er is, en `👀 Volg` werkt tijdens een spel.** Bestanden: `spel/ctx.gd` (nieuw `ctx.wacht_op`, `ctx.is_er`), `autoload/games.gd` (`beurt`, `verwacht`, `verwacht_dier`, volgen stopt bij start en stop), `autoload/hotel.gd` (alleen `komt_eraan` en de volgsectie), `autoload/hits.gd` (alleen de voorrangsregel: `data.spel`), `autoload/world.gd` (nieuw `World.blijf`), `games/zwembad` (alleen `_begin`), `sleutels`, `kraam`, `hinkel`, `oogst`, `weeg`, `tobbe` (`spel.gd` en `test_*.gd`), nieuw `tests/test_wacht.gd`, `tests/test_komt.gd`, `tests/test_games.gd`, specs `world.md` §2.5, §5.2, §5.3, §5.8, §6.3, `games-a.md` §4.5 en §6.5 D, `games-b.md` §0.12, §1.6, §1.12, §2.1, §3.6, §3.8, §3.9, §5.5, `games-c.md` §2.6 en §3.6. **De wens:** "Bij het zwembad is er geen volg optie om boef aan te komen tenzij ik eerst op 'terug' druk. Zorg dat de minigame pas begint wanneer het dier er is". **Wat het kind nu ziet:** start een spel terwijl het dier van de beurt nog elders in het hotel is, dan staan alleen de spullen van het spel er (bad met streepjes, kraam met waren, stenen, tafel, weegschaal) en hangt bij de deur het hotelwolkje `🐶 Boef komt eraan` met balk en `👀 Volg` — ook tijdens het spel; geen kaart, strook, prijskaartje, munt, haakje of cijfer. `👀 Volg` loopt met hem mee door elke deur (de spelbalk blijft staan, de deuren niet) en eindigt in de kamer van het spel; daar komt de eerste vraag zodra hij op zijn plek staat. Staat hij er al, of in rustmodus, dan meteen, zoals vroeger. Sleutelbord: dat geldt voor elke sleutel. Tobbe: badgasten van elders krijgen hun knopje pas als ze er zijn (de oude herteken-lus van 8 × 0,9 s is weg). Wekker: niemand loopt, niets veranderd. **Besluiten:** tijdens een spel volg je alleen het dier waarop het spel wacht (anders zou elk wandelend dier het spel storen); een spel dat stopt beëindigt het volgen waar de camera is, een verdwenen dier brengt de camera terug naar het spel; een half gezwommen baan wacht op het dek waar `stop()` de zwemmer neerzette, het antwoord zet hem daarna terug op zijn meter (bestaand gedrag); R1 telt voortaan vanaf de aankomst. **N11 vervalt** (de eigenaar overrulet "de som staat er meteen"), N3 stap 1 is omgedraaid. **Testverwachtingen die veranderden:** zwembad `test_de_vraag_staat_er_voor_de_duik` → `test_de_vraag_wacht_op_de_zwemmer` + `test_in_rust_staat_de_vraag_er_meteen` (de zwemmer staat nu op het dek als de vraag komt); hinkel `test_kindtekst_staat_woordelijk_in_het_bestand` zonder "%s komt eraan"/"Tel straks mee"/"Komt eraan"; `tests/test_games.gd` `test_het_dier_van_de_beurt_gaat_rond` zet de zwemmers eerst terug aan de balie; sleutels en kraam draaien hun reeks in rustmodus (de wandeling naar de balie/toonbank is dan meteen klaar), en de dekkingstest van sleutels op 740 × 360 keurt de gestapelde strook apart (hij ligt op niets en laat de gast heel). **Open:** op de liggende telefoon (740 × 360) staat de gast aan de balie precies op de vloer naast de vraagkaart, dus de getallenstrook stapelt linksboven (`krap` in het bandenrooster) — zo stond het in het oude spel ook zodra hij er was; de kolom van de kaart is nu zo breed als de strook zodat die niet meer op het eerste haakje ligt, de echte plaats hoort bij het plaatsingswerk. Foto's: /tmp/rkn-wacht-shots/. Suite: 579 goed, 0 fout.

- 2026-09-23 (Claude Opus 5.5, tak `voerkar-tik`) — **EIGENAARSWENS 2026-09-23 — De voerkar met tikken: tik op de kar, dan op een deur; één wolkje bij de kar.** Bestanden: `games/voerkar/spel.gd`, `games/voerkar/test_voerkar.gd`, spec `games-a.md` §7.1, §7.6, §7.7. **De meldingen:** "De kar kan niet gesleept worden … Zorg dat je op de kar kan klikken en daarna op een deur" en "heel veel textboxen die allemaal klikbaar lijken". **Nu:** de knop hangt óp de kar (`op: "aan"`) en is een schakelaar: `🛒 Pak de kar` → vast `🛒 Je duwt de kar` (ingedrukt, ☝); zolang hij vast is dragen de deuren op weg naar een hongerige kamer een `👉` op hun bordje; een tik op een deur duwt kar én camera erdoor en de kar blijft vast; een tik op de vaste kar zet hem neer. **Beslissing:** een deur zonder vaste kar neemt de kar gewoon mee (geen dode tik, geen "nee" om te lezen; het alternatief "👉 Tik eerst op de kar" niet gekozen). Het bakje is alleen een knop waar vullen nu kan (kar in die kamer, kamer open — vast of neergezet). Eén wolkje bij de kar (`vk_zeg`, een wolkje doet niets bij een tik): `🍪 Breng <per> koekjes naar elke gast` (alleen als eerste instructie; het oude duw-wolkje), `👉 Tik op een deur`, `👉 Tik op het bakje` (bij het getekende bakje), `👉 Tik op de kar` (neergezet), `✅ Alle bakjes vol!`; tijdens het smulwolkje wacht het; aan het eind geen knop meer en het spel sluit zelf na 2,6 s (het slotwolkje sluit niet meer op een tik). Vervallen: `nog %d %s`, `kar is leeg`, `de voerkar is leeg`, `Sleep de kar naar een deur`, `Sleep de kar hierheen`. **Slepen:** in Firefox 155/156 niet stuk te krijgen (muis en touch, BiDi en echte X11-invoer, net als Chromium); waarschijnlijkste oorzaak bij de eigenaar: alleen het labeltje NAAST de kar was sleepbaar (de 0 %-regel houdt knoppen van hun ding af), wie de getekende kar pakte greep in het niets — nu hangt de knop óp de kar. Wél gevonden en voor de kar opgelost: Firefox meet `movementX` van een vinger vanaf de laatste muisplek, dus na één muisbeweging werd een trillende tik op de kar een sleep die nergens landde; de kar start zijn sleep nu zelf op de echte afstand (`force_drag`, 10 eenheden), in Firefox nagespeeld. **Gezien, buiten deze set:** (1) alle andere bronnen (`ui/bron.gd`) hebben hetzelfde Firefox-touchprobleem — oplossing daar: `_get_drag_data` pas na 10 eenheden echte afstand, of `force_drag`; (2) `Hits._vlak_van` meet een bakje zonder `Art.KOM_ANKER`: het vlak ligt ±88 × 70 rechtsonder het getekende bakje, dus "🍽 Leeg" hangt ernaast, de 0 %-toets meet het verkeerde vak en een sleep op het getekende bakje mist; (3) band-plaatsing ontwijkt alleen voorwerpen met een knop: het startwolkje kan over de zak in de keuken hangen. **Testverwachtingen bijgesteld:** `test_kindteksten_staan_er_verbatim` (oude zinnen weg, nieuwe erin, oude teksten mogen niet meer in de bron), `_hele_beurt` (nu helemaal getikt; `vk_duw` → `vk_zeg`, `vk_hier` weg, `vk_af` + tik → `vk_zeg` en wachten tot het spel zelf sluit), `test_kar_staat_na_de_ronde_weer_in_de_keuken` (wachten i.p.v. tik op `vk_af`), `test_els_blijft_en_de_ronde_straft_niet` (geen `vk_hier`; Els' zin in `vk_zeg`), `test_dekking_nul_in_vier_kaders` (vijf beelden: keuken, keuken vast, kamer1 bij het bakje, gevoerd + neergezet, gevoerd + vast), `test_slepen_de_kar_door_de_deur` (daarna vast), helper `_tik` weigert een knop die niet in beeld staat; zeven nieuwe tests (tik pakt, deur neemt mee, neerzetten, deur zonder vaste kar, na `stop()` geen 👉, trillende tik blijft tik, F4 voor de nieuwe zinnen). `DH_TEST_FILTER=voerkar` 25 goed, 0 fout; volle suite **568 goed, 0 fout**. Beelden (xvfb, 1536×760, 1024×768, 360×740, 740×360) en Firefox-proef: `/tmp/rkn-voerkar-shots/`. **Open:** `V4` ("🛒 Duw naar <kamer>" in één tik) botst met deze tikweg en moet opnieuw bekeken worden.

- 2026-09-24 (Claude Opus 5.5, tak `intro`, niet gemerged) — **EIGENAARSWENS 2026-09-23 — Een leuke intro voor de game.** Bestanden: nieuw `ui/intro.gd` en `tests/test_intro.gd` (met `.uid`), `scenes/main.gd`, `ui/teksten.gd`, `fonts/tekens.txt` en `fonts/Emoji.ttf` (👑 U+1F451 en ➕ U+2795 in de subset; alleen de emoji-subset herbouwd met dezelfde fontTools-opdracht als `fonts/maak-fonts.sh`, die zonder die twee byte-gelijk de oude oplevert), `tools/probe.js`, spec `world.md` §6.2 en §7.2. **De wens:** "Maak ook een leuke intro voor de game" (samen met "laat niet het prikbord zien als start. De gebruiker moet gewoon op de bel drukken" — dat deel en het nieuwe belgeluid doet de lead in `hotel.gd`/`snd.gd`). **Nu:** een vers spel (geen opslag, of `Nieuw spel`; nooit na `Verder spelen`, en de opslag krijgt er geen veld voor) begint met zes pagina's in de echte receptie, elk één zin op een groot kaartje: `👋 Welkom in het Dierenhotel!` · `👑 Jij bent de baas van het hotel!` · `🐾 Kijk, daar komen de gasten!` (Boef, Muis en Wolkje — de eerste drie van de wachtlijst — lopen binnen) · `✨ Elk dier heeft een wens.` (🛏 bed, 🍪 eten, 🧶 spelen boven hun dier) · `➕ Met sommen help je de dieren!` (de wolkjes munt-groen met ✓, de dieren blij, `hoera`) · `🔔 Druk op de bel voor Boef!` (ze lopen weer naar buiten, het hotel wordt schemerig op een lichtcirkel om de bel na, met een kloppende ring en een wippende pijl). Een tik in het licht drukt de echte belknop (zelfde geluid, zelfde `[probe] tik`, `Hotel.bel()`) en de intro is weg; een tik ernaast sluit zonder te bellen. Overal tikken of `Verder ▸` = volgende; `Overslaan ▸▸` op elke pagina; een verse pagina negeert 0,35 s lang tikken, zodat een dubbeltik geen pagina opeet. Rustmodus: dezelfde pagina's als stilstaande plaatjes (neergezet, geen fonkels, geen puls, geen overvloeiing; op de laatste pagina zijn ze meteen weg). De intro sluit het prikbord dat `Hotel.start()` op de ochtendronde opent, en de knoppen van het hotel wijken tijdens pagina 1–5 met `Hits.voorrang("intro")` (zoals voor een spel) en komen op de laatste pagina terug. De binnenkomst hangt aan één functie, `UiIntro._ingang()` (nu de gangdeur), voor de voordeur van tak `binnenkomst`. **Tests:** nieuw `tests/test_intro.gd` (13): intro bij een vers spel en na `Nieuw spel`, niet na `Verder spelen`; `Overslaan` sluit meteen en laat geen dier, wolkje of voorrang achter; een dubbeltik is één pagina; de zes zinnen op volgorde en wat de wereld doet; de laatste pagina zet het licht op de bel en een tik erin laat Boef aan de balie komen; een tik ernaast belt niet; de zinnen houden HOTEL.md §9 (≤ 8 woorden mét pictogram zoals `Ui.keur_regel` telt, ≤ 40 tekens, elk teken in de fontsubset) voor elke naam van de pool; op 1536×760, 1024×768, 360×740 en 740×360 elke pagina in beeld, knoppen ≥ 48×48, de zin op hooguit twee regels, geen letter onder 12 px, het kaartje nooit over het licht; rustmodus staat stil en zonder rustmodus beweegt het wél; een tweede intro houdt zijn podium; een schil die weggaat ruimt de intro op. Op beeld nagekeken op de vier schermen (elke pagina, ook rustmodus en na de bel) en in chromium met `tools/probe.js` (1024×768@2, 360×740@3, 740×360@3). **Zelf beslist:** (1) de laatste zin noemt de gast die de bel brengt in plaats van "voor je eerste gast": met het pictogram als woord zijn dat er 9. (2) 👑 en ➕ toegevoegd: 🎩 is het souvenirhoedje van de kraam, ⭐ de ster voor meedoen. (3) De laatste pagina heeft geen `Verder`: de bel is daar de knop. (4) Op de wenspagina's maken de naambordjes plaats voor de wolkjes — met bordje hing een wolkje onder zijn dier, dichter bij het dier ernaast. (5) Liggende telefoon (lager dan 450): het kaartje als één rij bovenin over de chroom, anders nam het de band van de wolkjes. (6) `tools/probe.js` tikt bij een verse start eerst `Overslaan ▸▸`, zet de bel vóór het prikbord in zijn voorkeur (de eerste taak, en een half inchecken houdt het bord na de herlaad dicht) en bevestigt `verder` op een knop die na de keuze echt gemeld is. **Bekend, niet veranderd:** op 740×360 meldt de probe 4 consolefouten — de bestaande waarschuwing `somkaart "ci_som": regel2 vervalt in de lage balk` van het inchecken, op `main` precies zo; de 2 gelekte CanvasItem-RID's en 4 ObjectDB-objecten bij het afsluiten van de suite staan ook zo op `main`; op 360×740 hangt het deurbordje "Gang" half in de lichtcirkel en het wolkje van Muis soms onder hem; de `tover` van pagina 1 is op het web stil (geen geluid vóór de eerste aanraking); niet op een echt toestel geprobeerd. 574 goed, 0 fout.

- 2026-09-24 (Claude Opus) — **Samenvoegen: de intro komt door de voordeur; een bakje wordt gemeten waar het getekend staat.** Bestanden: `ui/intro.gd`, `tests/test_intro.gd`, `autoload/hits.gd`. (1) De gasten van het verhaaltje kwamen door de gangdeur (de intro-tak kende de voordeur nog niet): `UiIntro._ingang()` neemt nu `Rooms.ingang("receptie")`, en elk dier komt binnen met zijn eigen binnenkomst (`World.kom_binnen`), net als een gast van de bel; `beweegt()` en de bewegingstest tellen de toestand `komt` mee. (2) `Hits._vlak_van` mat een bakje (`kom`) zonder `Art.KOM_ANKER`: het vak lag zo'n 88 × 70 eenheden rechtsonder het getekende bakje, "🍽 Leeg" hing ernaast en een sleep op het bakje viel mis (gevonden door de voerkar-tak). Nu meet het met hetzelfde anker als `scenes/kamer.gd`.

- 2026-09-24 (Claude Opus) — **Een tik op een bron blijft een tik, ook in Firefox.** Bestanden: `ui/bron.gd`, `tests/test_ui.gd`. De voerkar-tak vond het voor de kar: Firefox meet de `movementX` van een vinger vanaf de laatste plek van de muis, dus na één muisbeweging in de sessie kreeg het eerste trillinkje van een tik een `relative` van honderden eenheden, begon Godot daarop een sleep en was de tik weg. Dat gold voor elke bron (de buidel van de meubels, de munten van de kraam, de was, de tobbe, de sleutels). `UiBron` weigert Godots eigen start zolang de vinger nog binnen 10 eenheden van zijn indrukpunt is en begint de sleep zelf (`force_drag`) zodra hij echt verder is. Nieuwe test `test_een_trillende_tik_op_een_bron_blijft_een_tik` (het trillinkje blijft een tik, 40 eenheden verder is het een sleep).

- 2026-09-24 (Claude Opus 5.5, tak `zwembad-beter`) — **EIGENAARSWENS 2026-09-23 — Zwembad: getallen boven 10, een warme bots, en na een bots opnieuw rekenen; het hek langs de lange kant.** Bestanden: `games/zwembad/spel.gd`, `games/zwembad/beurt.gd`, `games/zwembad/test_zwembad.gd`, `autoload/rooms.gd` (alleen het zwembadblok en `_bouw_zwembad`), `tests/test_rooms.gd`, specs `games-b.md` §1.1, §1.6, §1.7, §1.8, §1.11, §1.12 en `world.md` §1. **De wens:** "Getallen boven 10 klikken wordt niet afgehandeld en het dier dat zich stoot is slecht geanimeerd. Bij stoten moet de speler ook opnieuw rekenen met een andere afstand" en "zet het hek aan de bovenste zijkant van het zwembad niet aan de korte kant bij de startblokken". **Getallen boven 10 — eerst nagemeten:** echte tikken (muis én touch, door het venster van de echte schil) op elke knop van de strook, op 1536×760 en de vier ticketschermen, band 3/4/5, tot vijf kaarten per baan: 258 tikken, 0 niet afgehandeld; in de webexport (Chromium, `speel.js`, 1536×760 en 360×740) kwamen 16 en 20 ook aan. Er is dus geen invoerfout. Wat de eigenaar zag: op band 3 (max 10 per keer) is elk getal boven 10 óf te veel (hij zwemt maar 10, "hooguit 10 m") óf voorbij de rest — en dat laatste stuurde hem naar de wand en terug naar precies dezelfde meter, met dezelfde vraag en dezelfde vier knoppen: op het scherm veranderde er niets. Dat is opgelost bij de oorzaak (`_bots_en_terug` zette `p` terug op `p_voor`), zie hieronder. **De bots nu:** hij remt af en raakt de verre wand met zijn neus (niet meer met zijn midden op `x(L)` en zijn kop op de tegels), `au`, spetters over de wand, `💛 Au!` (1,2 s, staat stil), zijn kop duikt tegen de wand (`snuif`) en komt duizelig omhoog (`kijk`) met wit-roze twinkelingetjes rond zijn kop (nooit sterkleur), dan duwt de wand hem terug door het water terwijl het getal op zijn rug terugtelt, een slagje vooruit draait hem weer naar de wand, en er komt een NIEUWE vraag: `🙃 <naam> botste terug naar <p> meter` / `Nog hoeveel meter?` / `<L> − <p> =` met de knoppen uit `keuzeGetallen(L, M, p, band, leg)`. Rustmodus: stil tegen de wand zolang `💛 Au!` staat, dan meteen op zijn nieuwe meter, geen deeltjes. **De regel** (`ZwembadBeurt.bots_plek`, bevroren kern onaangeroerd): een vijfde tot een derde van de baan terug van de wand, nooit dezelfde afstand, nooit op of voorbij de wand of vóór de start, altijd in één slag te halen, vóór de meter waar hij begon als de baan daar ruimte voor laat, gekozen door de eigen LCG van het zwembad op (L, M, p, leg) — voorbeeld: 16 m, max 10, eerst 10 m, dan 16 getikt → terug naar 11 m, vraag `16 − 11 =` met 16 / 10 / 5 / 15. De opslag heeft die meter al vóór hij beweegt. Er gaat niets af: geen ster, geen etappe. **Het hek:** alleen nog langs de lange achterkant (`hekx` op z 4 vanaf x 32), geen zijhek bij het startblok; de rozenboog blijft de poort. **Ook gevonden en hersteld:** (1) bij "precies" stond zijn kop ook op de tegels — elke slag tot de wand stopt nu met de neus ertegen; (2) na de overkant liep hij het water uit alsof hij erop stond en stond daarna 7 voxels in het dek verzonken (`World._aangekomen` houdt de `spring_land` van de duik vast voor elke latere aankomst) — hij hopt nu over de rand het dek op, wat die waarde herstelt; (3) zijn dekplek `over` stond in de voet van de vlag: (132, 56) → (116, 56); (4) de strook van het zwembad meldde zich niet aan `speel.js` — nu `[probe] zb_som_keuzes=`. **Gemeld, niet aangeraakt:** (a) de World-fout zelf: `_aangekomen` zou `spring_land` alleen mogen gebruiken als de afgelopen opdracht een sprong was (`if d.beweeg_pose == "spring" and d.spring_land < 0.0`); na `stop()` → `_nood_uit` (`World.zet` op het dek) zakt hij bij zijn volgende aankomst nog steeds weg; (b) de instap (de parallelle start-taak): na elke hop van het trappetje zet `_aangekomen` hem terug op de vloer, tijdens de adem van 0,7 s staat hij op vloerhoogte ín het startblok en duikt hij vanaf de vloer, en na de duik (x 20,5) zwemt hij 2,5 voxel achteruit naar x(0) = 18 en draait daarbij om; (c) zijn naambordje zweeft tijdens het zwemmen over het nieuwe achterhek (dat komt door de hoogte van het bordje; verder naar achteren helpt niet, nagemeten tot z −12); (d) op een telefoon met een gast wachtend onder het startblok hangt de 🏊-knop over de eerste hekpalen (de drie andere kanten zijn bezet). **Testverwachtingen bijgesteld:** `test_een_te_ver_antwoord_eindigt_de_beurt_niet` (was: terug op dezelfde meter met dezelfde som en "Te ver, hij tikt de rand"; nu: `bots_plek`, een andere afstand, de nieuwe som, en daarna precies met ster), `test_de_zinnen_staan_er_woordelijk` (`regel_bots_terug(naam, p)`), `test_rooms` `dek.over`. **Nieuwe tests:** `test_elke_knop_op_de_strook_werkt` (echte tikken door de schil op vier schermen, band 3/4/5, eerste en tweede kaart), `test_de_botsplek_volgt_de_regel`, `test_een_slag_tot_de_wand_stopt_met_de_neus_ertegen`, `test_de_bots_in_rustmodus`, `test_de_bots_als_film`, `test_na_de_overkant_staat_hij_op_het_dek`, `test_het_hek_staat_langs_de_lange_kant`. Suite: **568 goed, 0 fout** (`main` 48268e8: 561; de lekmeldingen bij het afsluiten — 4 ObjectDB, 2 CanvasItem — staan op `main` precies zo).
...
- 2026-09-24 (Claude Opus) — **EIGENAARSWENS 2026-09-24 — De voordeur staat open bij het roze tapijt.** Bestanden: `autoload/rooms.gd` (receptieblok, `bouw_af`, `deur_hoog`), `autoload/world.gd`, `art/decor_hotel.gd`, `tests/test_rooms.gd`, `tests/test_binnenkomst.gd`, specs `world.md` §1.2 en §1.3, `art-sound-rules.md` §8. **De wens:** "Nee zet de lobby deur waar het tapijt is maar alleen de uitlijn en laat hem open zodat je erdoorheen kan kijken en het tapijt zien ... En maak hem wat groter dan de andere deuren". **Nu:** de voordeur staat in de voorrand van de lobby (`wand: "voor"`, z = 120), midden op het roze tapijt (x 62..82), en is alleen een omlijsting (`voordeuromlijst`: twee witte stijlen, een latei, de drempel), open, eerst 20 × 32 en na "Mooi maak de deur nog iets groter" 26 breed en 38 hoog (een deur in een muur is 12 × 26). Een gast verschijnt op de drempel en stapt het tapijt op, recht naar de balie; de open omlijsting gaat nooit dicht en geeft geen deurgeluid (`ingang.open`, `World.ingang_open` altijd waar). De dichte deur en het matje op de achterwand zijn weg (de modellen bestaan nog). De intro gebruikt dezelfde ingang. **Testverwachtingen bijgesteld:** `test_rooms` voordeur buiten de deurgraaf (voorrand, breder en hoger dan een deur, midden voor het tapijt, alleen de omlijsting) en `_deur_dekking` kent de voorrand en telt de eigen omlijsting niet mee; `test_binnenkomst` de deur staat open en gaat nooit dicht (vier plekken), en de kaart en haar strook tellen niet mee voor wat de voordeur bedekt — een kaart mag over de wereld (op 740×360 hangt de incheckvraag over de voorkant van de lobby).
...
| **3 — Het balkanker** | `S5` `B3` `V2` `V5` `N8` `N9` `K1` `K3` `R2` | 9 / 9 |
| **4 — Grote letters** | `B4` `V3` `N10` `N12` `N13` `K2` `R3` | 1 / 7 |
