...
- **2026-09-16**: the HTML demos (`demos/dierenhotel`, `silverhoof`, `waggletail`), the PyCharm stub `main.py` and the oracle-generator scripts in `tests/gouden` are deleted at the owner's request; the oracle data stays and no test runs JavaScript. Everything is in git history at 7945251. The doc comments "Port of demos/…" in the GDScript files are historical pointers, kept on purpose.
